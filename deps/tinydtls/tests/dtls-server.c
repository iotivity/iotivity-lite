...
static const unsigned char g_server_certificate[SERVER_CRT_LEN] = {
  0x00, 0x01, 0x24, 0x30, 0x82, 0x01, 0x20, 0x30, 0x81, 0xc4, 0xa0, 0x03, 0x02,
  0x01, 0x01, 0x02, 0x02, 0x02, 0x37, 0x30, 0x0c, 0x06, 0x08, 0x2a, 0x86, 0x48,
  ...
  
};

// default server's key pair
static const unsigned char x509_priv_key[] = {
  0xaa, 0xa3, 0x46, 0xf1, 0x3c, 0x56, 0x5d, 0x08, 0x5e, 0x59, 0xba,
  0x7f, 0xd2, 0x21, 0x62, 0xc6, 0xcc, 0x5d, 0xfa, 0x3f, 0xb5, 0x25,
  0xa9, 0x89, 0x4f, 0x32, 0xe8, 0x2a, 0xe0, 0xee, 0x9b, 0x4c
};
...
dtls_handle_read(struct dtls_context_t *ctx)
{
  ...
  len =
    recvfrom(*fd, buf, sizeof(buf), MSG_TRUNC, &session.addr.sa, &session.size);
...
usage(const char *program, const char *version)
{
  ...
  fprintf(stderr,
          "%s v%s -- DTLS server implementation\n"
...
/* fill extra field for 4.4BSD-based systems (see RFC 3493, section 3.4) */
  ...
  while ((opt = getopt(argc, argv, "A:p:v:a:")) != -1) {
    ...
    case 'x': {
      ssize_t result =
        read_from_file(optarg, x509_server_cert, DTLS_MAX_CERT_SIZE);
      if (result < 0) {
    ...
    case 'r': {
      ssize_t result =
        read_from_file(optarg, x509_server_priv, DTLS_PRIVATE_KEY_SIZE + 1);
      if (result < 0) {
  ...
  } else if (!(x509_server_cert_len || x509_server_priv_is_set ||
               x509_ca_pub_is_set)) {
