...
static inline void
update_hash(uint8 *record, size_t rlength, uint8 *data, size_t data_length)
{
  int i;
...
/* temporarily store hash status for roll-back after finalize */
  ...
  memcpy(statebuf + sizeof(md5_state_t), hs_hash[1], sizeof(SHA_CTX));
#elif DTLS_VERSION == 0xfefd
...
/* restore hash status */
  ...
  memcpy(hs_hash[1], statebuf + sizeof(md5_state_t), sizeof(SHA_CTX));
#elif DTLS_VERSION == 0xfefd
...
int
pcap_verify(dtls_security_parameters_t *sec, int is_client,
            ...
            const unsigned char *cleartext, size_t cleartext_length)
{
  ...
  dtls_hmac_init(&hmac_ctx, is_client ? dtls_kb_client_mac_secret(sec)
  ...
  dtls_mac(&hmac_ctx, record, /* the pre-filled record header */
           cleartext, cleartext_length, mac);

  ok = memcmp(mac, cleartext + cleartext_length, dtls_kb_digest_size(sec)) == 0;
#ifndef NDEBUG
               ...
               uint8 **cleartext, size_t *clen)
{
  ...
  case AES128: /* TLS_PSK_WITH_AES128_CBC_SHA */
    ...
    *clen = length - sizeof(dtls_record_header_t);
    ...
    res = dtls_decrypt(cipher, (uint8 *)packet + sizeof(dtls_record_header_t),
                       *clen, buf, NULL, 0);
...
{
  static int n = 0;
  static unsigned char initial_hello[] = { 0x16, 0xfe, 0xff, 0x00, 0x00, 0x00,
...
#define verify_data_length 12
  ...
  while (length) {
    ...
    res = decrypt_verify(is_client, packet, rlen, &data, &data_length);
    ...
    if (packet[0] == 20 && data[0] == 1) { /* ChangeCipherSpec */
      ...
      printf("\n");
      master_secret_len = dtls_prf(
        pre_master_secret, pre_master_len, (unsigned char *)"master secret", 13,
        dtls_kb_client_iv(OTHER_CONFIG), 32, dtls_kb_server_iv(OTHER_CONFIG),
        32, master_secret, DTLS_MASTER_SECRET_LENGTH);
      ...
      dtls_prf(
        master_secret, master_secret_len, (unsigned char *)"key expansion", 13,
        dtls_kb_server_iv(OTHER_CONFIG), 32, dtls_kb_client_iv(OTHER_CONFIG),
        32, OTHER_CONFIG->key_block, dtls_kb_size(OTHER_CONFIG));

      OTHER_CONFIG->read_cipher = dtls_cipher_new(
        OTHER_CONFIG->cipher, dtls_kb_client_write_key(OTHER_CONFIG),
      ...
      OTHER_CONFIG->write_cipher = dtls_cipher_new(
        OTHER_CONFIG->cipher, dtls_kb_server_write_key(OTHER_CONFIG),
        dtls_kb_key_size(OTHER_CONFIG));
      ...
      printf("  client_IV:\t\t");
      dump(dtls_kb_client_iv(CURRENT_CONFIG), dtls_kb_iv_size(CURRENT_CONFIG));
      printf("\n");

      printf("  server_IV:\t\t");
      dump(dtls_kb_server_iv(CURRENT_CONFIG), dtls_kb_iv_size(CURRENT_CONFIG));
      printf("\n");
    }
    ...
    if (packet[0] == 22) {
      if (data[0] == 20) { /* Finished */
        ...
        update_hash((unsigned char *)packet, sizeof(dtls_record_header_t), data,
                    data_length);

        dtls_prf(master_secret, master_secret_len,
                 is_client ? (unsigned char *)"client finished"
                           : (unsigned char *)"server finished",
                 15, hash_buf, sizeof(hash_buf), NULL, 0,
                 data + sizeof(dtls_handshake_header_t), verify_data_length);
        printf("verify_data:\n");
...
init()
{
  ...
  if (!pre_master_len) {
    ...
    pre_master_len = dtls_pre_master_secret((unsigned char *)"secretPSK", 9,
                                            pre_master_secret);
  }
}
...
main(int argc, char **argv)
{
  ...
  static struct option opts[] = { { "psk", 1, 0, 'p' }, { 0, 0, 0, 0 } };
  ...
  while (1) {
    ...
    switch (c) {
    case 'p':
      pre_master_len = dtls_pre_master_secret(
        (unsigned char *)optarg, strlen(optarg), pre_master_secret);
      break;
    }
  }
