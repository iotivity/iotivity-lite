...
#define Curve_B_1                                                              \
  {                                                                            \
    0x45, 0xFA, 0x65, 0xC5, 0xAD, 0xD4, 0xD4, 0x81, 0x9F, 0xF8, 0xAC, 0x65,    \
      0x8B, 0x7A, 0xBD, 0x54, 0xFC, 0xBE, 0x97, 0x1C                           \
  }
#define Curve_B_2                                                              \
  {                                                                            \
...
#define Curve_G_1                                                              \
  {                                                                            \
    ...
    {                                                                          \
      0x32, 0xFB, 0xC5, 0x7A, 0x37, 0x51, 0x23, 0x04, 0x12, 0xC9, 0xDC, 0x59,  \
        0x7D, 0x94, 0x68, 0x31, 0x55, 0x28, 0xA6, 0x23                         \
    }                                                                          \
  }
...
#define Curve_G_4                                                              \
  {                                                                            \
    ...
    {                                                                          \
      0xB8, 0xD4, 0x10, 0xFB, 0x8F, 0xD0, 0x47, 0x9C, 0x19, 0x54, 0x85, 0xA6,  \
        0x48, 0xB4, 0x17, 0xFD, 0xA8, 0x08, 0x11, 0x0E, 0xFC, 0xFB, 0xA4,      \
        0x5D, 0x65, 0xC4, 0xA3, 0x26, 0x77, 0xDA, 0x3A, 0x48                   \
    }                                                                          \
  }

#define Curve_N_1                                                              \
  {                                                                            \
    0x57, 0x22, 0x75, 0xCA, 0xD3, 0xAE, 0x27, 0xF9, 0xC8, 0xF4, 0x01, 0x00,    \
...
static uECC_word_t vli_sub(uECC_word_t *p_result, uECC_word_t *p_left,
                           uECC_word_t *p_right);
static void vli_mult(uECC_word_t *p_result, uECC_word_t *p_left,
                     uECC_word_t *p_right);
static void vli_modAdd(uECC_word_t *p_result, uECC_word_t *p_left,
                       uECC_word_t *p_right, uECC_word_t *p_mod);
static void vli_modSub(uECC_word_t *p_result, uECC_word_t *p_left,
                       uECC_word_t *p_right, uECC_word_t *p_mod);
...
static void vli_modMult_fast(uECC_word_t *p_result, uECC_word_t *p_left,
                             uECC_word_t *p_right);
static void vli_modInv(uECC_word_t *p_result, uECC_word_t *p_input,
                       uECC_word_t *p_mod);
#if uECC_SQUARE_FUNC
...
int uECC_make_key_impl(uint8_t p_publicKey[uECC_BYTES * 2],
                       uint8_t p_privateKey[uECC_BYTES]);
int uECC_shared_secret_impl(const uint8_t p_publicKey[uECC_BYTES * 2],
                            const uint8_t p_privateKey[uECC_BYTES],
                            uint8_t p_secret[uECC_BYTES]);
int uECC_sign_impl(const uint8_t p_privateKey[uECC_BYTES],
                   const uint8_t p_hash[uECC_BYTES],
                   uint8_t p_signature[uECC_BYTES * 2]);
int uECC_verify_impl(const uint8_t p_publicKey[uECC_BYTES * 2],
                     const uint8_t p_hash[uECC_BYTES],
                     const uint8_t p_signature[uECC_BYTES * 2]);
int uECC_ecdhe_impl(const uint8_t p_public_key_in[uECC_BYTES * 2],
                    uint8_t p_public_key_out[uECC_BYTES * 2],
                    uint8_t p_secret[uECC_BYTES]);
int uECC_get_pubkey_impl(const uint8_t p_key_handle[uECC_BYTES],
...
#include <string.h>
static int
default_RNG(uint8_t *p_dest, unsigned p_size)
{
...
{
  HCRYPTPROV l_prov;
  if (!CryptAcquireContext(&l_prov, NULL, NULL, PROV_RSA_FULL,
                           CRYPT_VERIFYCONTEXT)) {
    return 0;
  }
...
void
uECC_set_rng(uECC_RNG_Function p_rng)
{
  g_rng = p_rng;
}
...
void
uECC_set_make_key_cb(uECC_make_key_Function p_make_key_cb)
{
  g_make_key_cb = p_make_key_cb;
}

static uECC_shared_secret_Function g_shared_secret_cb =
  &uECC_shared_secret_impl;

void
uECC_set_shared_secret_cb(uECC_shared_secret_Function p_shared_secret_cb)
{
...
void
uECC_set_sign_cb(uECC_sign_Function p_sign_cb)
{
  g_sign_cb = p_sign_cb;
}
...
void
uECC_set_verify_cb(uECC_verify_Function p_verify_cb)
{
...
void
uECC_set_ecdhe_cb(uECC_ecdhe_Function p_ecdhe_cb)
{
...
void
uECC_set_get_pubkey_cb(uECC_get_pubkey_Function p_get_pubkey_cb)
{
...
#if !asm_isZero
static uECC_word_t
vli_isZero(const uECC_word_t *p_vli)
{
  wordcount_t i;
  for (i = 0; i < uECC_WORDS; ++i) {
    if (p_vli[i]) {
...
#if !asm_testBit
static uECC_word_t
vli_testBit(const uECC_word_t *p_vli, bitcount_t p_bit)
{
  return (p_vli[p_bit >> uECC_WORD_BITS_SHIFT] &
          ((uECC_word_t)1 << (p_bit & uECC_WORD_BITS_MASK)));
}
#endif
...
#if !asm_numBits
static wordcount_t
vli_numDigits(const uECC_word_t *p_vli, wordcount_t p_maxWords)
{
  swordcount_t i;
...
static bitcount_t
vli_numBits(const uECC_word_t *p_vli, wordcount_t p_maxWords)
{
  uECC_word_t i;
  uECC_word_t l_digit;
...
#if !asm_set
static void
vli_set(uECC_word_t *p_dest, const uECC_word_t *p_src)
{
  wordcount_t i;
  for (i = 0; i < uECC_WORDS; ++i) {
    p_dest[i] = p_src[i];
...
#if !asm_cmp
static cmpresult_t
vli_cmp(uECC_word_t *p_left, uECC_word_t *p_right)
{
  swordcount_t i;
...
#if !asm_add
static uECC_word_t
vli_add(uECC_word_t *p_result, uECC_word_t *p_left, uECC_word_t *p_right)
{
  uECC_word_t l_carry = 0;
  wordcount_t i;
  for (i = 0; i < uECC_WORDS; ++i) {
...
#if !asm_sub
static uECC_word_t
vli_sub(uECC_word_t *p_result, uECC_word_t *p_left, uECC_word_t *p_right)
{
...
static void
muladd(uECC_word_t a, uECC_word_t b, uECC_word_t *r0, uECC_word_t *r1,
       uECC_word_t *r2)
{
...
#if !asm_mult
static void
vli_mult(uECC_word_t *p_result, uECC_word_t *p_left, uECC_word_t *p_right)
{
  uECC_word_t r0 = 0;
  uECC_word_t r1 = 0;
  uECC_word_t r2 = 0;
...
#if !asm_square
static void
mul2add(uECC_word_t a, uECC_word_t b, uECC_word_t *r0, uECC_word_t *r1,
        uECC_word_t *r2)
{
#if uECC_WORD_SIZE == 8 && !SUPPORTS_INT128
  uint64_t a0 = a & 0xffffffffull;
...
static void
vli_square(uECC_word_t *p_result, uECC_word_t *p_left)
{
  uECC_word_t r0 = 0;
  uECC_word_t r1 = 0;
  uECC_word_t r2 = 0;
...
#if !asm_modAdd
static void
vli_modAdd(uECC_word_t *p_result, uECC_word_t *p_left, uECC_word_t *p_right,
           uECC_word_t *p_mod)
{
  uECC_word_t l_carry = vli_add(p_result, p_left, p_right);
  if (l_carry ||
      vli_cmp(p_result, p_mod) >= 0) { /* p_result > p_mod (p_result = p_mod +
                                          remainder), so subtract p_mod to get
                                          remainder. */
...
#if !asm_modSub
static void
vli_modSub(uECC_word_t *p_result, uECC_word_t *p_left, uECC_word_t *p_right,
           uECC_word_t *p_mod)
{
 ...
 */
static void omega_mult(uECC_word_t *RESTRICT p_result,
                       uECC_word_t *RESTRICT p_right);
...
static void
vli_mmod_fast(uECC_word_t *RESTRICT p_result, uECC_word_t *RESTRICT p_product)
{
...
#if uECC_WORD_SIZE == 1
static void
omega_mult(uint8_t *RESTRICT p_result, uint8_t *RESTRICT p_right)
{
  uint8_t l_carry;
  uint8_t i;
...
#elif uECC_WORD_SIZE == 4
static void
omega_mult(uint32_t *RESTRICT p_result, uint32_t *RESTRICT p_right)
{
  uint32_t l_carry;
  unsigned i;
...
#if uECC_WORD_SIZE == 1
static void
vli_mmod_fast(uint8_t *RESTRICT p_result, uint8_t *RESTRICT p_product)
{
  ...
  l_tmp[0] = l_tmp[1] = l_tmp[2] = l_tmp[3] = l_tmp[4] = l_tmp[5] = l_tmp[6] =
    l_tmp[7] = 0;
  ...
  l_tmp[16] = l_tmp[17] = l_tmp[18] = l_tmp[19] = l_tmp[20] = l_tmp[21] =
    l_tmp[22] = l_tmp[23] = 0;
  l_carry += vli_add(p_result, p_result, l_tmp);
...
#elif uECC_WORD_SIZE == 4
static void
vli_mmod_fast(uint32_t *RESTRICT p_result, uint32_t *RESTRICT p_product)
{
...
#else
static void
vli_mmod_fast(uint64_t *RESTRICT p_result, uint64_t *RESTRICT p_product)
{
...
omega_mult(uint8_t *RESTRICT p_result, uint8_t *RESTRICT p_right)
{
  ...
  p_result[4 + uECC_WORDS] =
    vli_add(p_result + 4, p_result + 4, p_right); /* add the 2^32 multiple */
}
#elif uECC_WORD_SIZE == 4
static void
omega_mult(uint32_t *RESTRICT p_result, uint32_t *RESTRICT p_right)
{
  ...
  p_result[1 + uECC_WORDS] =
    vli_add(p_result + 1, p_result + 1, p_right); /* add the 2^32 multiple */
}
#else
static void
omega_mult(uint64_t *RESTRICT p_result, uint64_t *RESTRICT p_right)
{
  uECC_word_t r0 = 0;
  uECC_word_t r1 = 0;
  uECC_word_t r2 = 0;
...
static void
vli_modMult_fast(uECC_word_t *p_result, uECC_word_t *p_left,
                 uECC_word_t *p_right)
{
...
static void
vli_modSquare_fast(uECC_word_t *p_result, uECC_word_t *p_left)
{
  uECC_word_t l_product[2 * uECC_WORDS];
...
#if !asm_modInv
static void
vli_modInv(uECC_word_t *p_result, uECC_word_t *p_input, uECC_word_t *p_mod)
{
...
static cmpresult_t
EccPoint_isZero(EccPoint *p_point)
{
...
static void
EccPoint_double_jacobian(uECC_word_t *RESTRICT X1, uECC_word_t *RESTRICT Y1,
...
static void
apply_z(uECC_word_t *RESTRICT X1, uECC_word_t *RESTRICT Y1,
...
/* P = (x1, y1) => 2P, (x2, y2) => P' */
static void
XYcZ_initial_double(uECC_word_t *RESTRICT X1, uECC_word_t *RESTRICT Y1,
...
*/
static void
XYcZ_add(uECC_word_t *RESTRICT X1, uECC_word_t *RESTRICT Y1,
         uECC_word_t *RESTRICT X2, uECC_word_t *RESTRICT Y2)
{
...
*/
static void
XYcZ_addC(uECC_word_t *RESTRICT X1, uECC_word_t *RESTRICT Y1,
          uECC_word_t *RESTRICT X2, uECC_word_t *RESTRICT Y2)
{
  /* t1 = X1, t2 = Y1, t3 = X2, t4 = Y2 */
  uECC_word_t t5[uECC_WORDS];
...
EccPoint_mult(EccPoint *RESTRICT p_result, EccPoint *RESTRICT p_point,
              const uECC_word_t *RESTRICT p_scalar,
              const uECC_word_t *RESTRICT p_initialZ, bitcount_t p_numBits)
{
  /* R0 and R1 */
...
static void
vli_nativeToBytes(uint8_t *RESTRICT p_dest, const uint8_t *RESTRICT p_src)
{
  uint8_t i;
...
static void
vli_nativeToBytes(uint8_t *p_bytes, const uint32_t *p_native)
{
  unsigned i;
  for (i = 0; i < uECC_WORDS; ++i) {
...
static void
vli_bytesToNative(uint32_t *p_native, const uint8_t *p_bytes)
{
  unsigned i;
  for (i = 0; i < uECC_WORDS; ++i) {
...
static void
vli_nativeToBytes(uint8_t *p_bytes, const uint64_t *p_native)
{
  unsigned i;
  for (i = 0; i < uECC_WORDS; ++i) {
...
static void
vli_bytesToNative(uint64_t *p_native, const uint8_t *p_bytes)
{
  unsigned i;
  for (i = 0; i < uECC_WORDS; ++i) {
...
int
uECC_make_key(uint8_t p_publicKey[uECC_BYTES * 2],
              uint8_t p_privateKey[uECC_BYTES])
{
...
int
uECC_shared_secret(const uint8_t p_publicKey[uECC_BYTES * 2],
                   const uint8_t p_privateKey[uECC_BYTES],
                   uint8_t p_secret[uECC_BYTES])
{
  // Check for a valid function pointer
...
int
uECC_sign(const uint8_t p_privateKey[uECC_BYTES],
...
int
uECC_verify(const uint8_t p_publicKey[uECC_BYTES * 2],
            const uint8_t p_hash[uECC_BYTES],
            const uint8_t p_signature[uECC_BYTES * 2])
{
  // Check for a valid function pointer
...
int
uECC_ecdhe(const uint8_t p_public_key_in[uECC_BYTES * 2],
           uint8_t p_public_key_out[uECC_BYTES * 2],
           uint8_t p_secret[uECC_BYTES])
{
  // Check for a valid function pointer
...
int
uECC_get_pubkey(const uint8_t p_key_handle[uECC_BYTES],
                uint8_t p_public_key[uECC_BYTES * 2])
{
  // Check for a valid function pointer
...
int
uECC_ecdhe_impl(const uint8_t p_public_key_in[uECC_BYTES * 2],
                uint8_t p_public_key_out[uECC_BYTES * 2],
                uint8_t p_secret[uECC_BYTES])
{
  return 0;
...
int
uECC_get_pubkey_impl(const uint8_t p_key_handle[uECC_BYTES],
                     uint8_t p_public_key[uECC_BYTES * 2])
{
  return 0;
}
...
int
uECC_make_key_impl(uint8_t p_publicKey[uECC_BYTES * 2],
                   uint8_t p_privateKey[uECC_BYTES])
{
  EccPoint l_public;
  ...
  do {
  repeat:
    if (!g_rng((uint8_t *)l_private, sizeof(l_private)) ||
        (l_tries++ >= MAX_TRIES)) {
      return 0;
    }
...
/* Make sure the private key is in the range [1, n-1]. */
    ...
    EccPoint_mult(&l_public, &curve_G, l_private, 0,
                  vli_numBits(l_private, uECC_WORDS));
...
int
uECC_shared_secret_impl(const uint8_t p_publicKey[uECC_BYTES * 2],
                        const uint8_t p_privateKey[uECC_BYTES],
                        uint8_t p_secret[uECC_BYTES])
{
  EccPoint l_public;
  ...
  EccPoint_mult(&l_product, &l_public, l_private,
                (vli_isZero(l_random) ? 0 : l_random),
                vli_numBits(l_private, uECC_WORDS));
...
void
uECC_compress(const uint8_t p_publicKey[uECC_BYTES * 2],
...
void
uECC_decompress(const uint8_t p_compressed[uECC_BYTES + 1],
...
static uECC_word_t
vli_isZero_n(const uECC_word_t *p_vli)
{
...
static void
vli_set_n(uECC_word_t *p_dest, const uECC_word_t *p_src)
{
...
static cmpresult_t
vli_cmp_n(uECC_word_t *p_left, uECC_word_t *p_right)
{
...
static void
vli_rshift1_n(uECC_word_t *p_vli)
{
  vli_rshift1(p_vli);
...
static uECC_word_t
vli_add_n(uECC_word_t *p_result, uECC_word_t *p_left, uECC_word_t *p_right)
{
  uECC_word_t l_carry = vli_add(p_result, p_left, p_right);
  uECC_word_t l_sum =
    p_left[uECC_N_WORDS - 1] + p_right[uECC_N_WORDS - 1] + l_carry;
...
static uECC_word_t
vli_sub_n(uECC_word_t *p_result, uECC_word_t *p_left, uECC_word_t *p_right)
{
  uECC_word_t l_borrow = vli_sub(p_result, p_left, p_right);
  uECC_word_t l_diff =
    p_left[uECC_N_WORDS - 1] - p_right[uECC_N_WORDS - 1] - l_borrow;
...
static void
vli_mult_n(uECC_word_t *p_result, uECC_word_t *p_left, uECC_word_t *p_right)
{
  uECC_word_t r0 = 0;
  uECC_word_t r1 = 0;
  uECC_word_t r2 = 0;
...
static void
vli_modAdd_n(uECC_word_t *p_result, uECC_word_t *p_left, uECC_word_t *p_right,
             uECC_word_t *p_mod)
{
...
static void
vli_modInv_n(uECC_word_t *p_result, uECC_word_t *p_input, uECC_word_t *p_mod)
{
...
static void
vli2_rshift1_n(uECC_word_t *p_vli)
{
  vli_rshift1_n(p_vli);
...
static uECC_word_t
vli2_sub_n(uECC_word_t *p_result, uECC_word_t *p_left, uECC_word_t *p_right)
{
  uECC_word_t l_borrow = 0;
  wordcount_t i;
...
static void
vli_modMult_n(uECC_word_t *p_result, uECC_word_t *p_left, uECC_word_t *p_right)
{
  ...
  for (i = 0; i <= ((((bitcount_t)uECC_N_WORDS) << uECC_WORD_BITS_SHIFT) +
    ...
    uECC_word_t l_borrow =
      vli2_sub_n(v[1 - l_index], v[l_index], l_modMultiple);
...
static void
vli2_rshift1(uECC_word_t *p_vli)
{
  vli_rshift1(p_vli);
...
static uECC_word_t
vli2_sub(uECC_word_t *p_result, uECC_word_t *p_left, uECC_word_t *p_right)
{
  uECC_word_t l_borrow = 0;
  wordcount_t i;
...
int
uECC_sign_impl(const uint8_t p_privateKey[uECC_BYTES],
               const uint8_t p_hash[uECC_BYTES],
               uint8_t p_signature[uECC_BYTES * 2])
{
  ...
  repeat:
    ...
    /* make sure that we don't leak timing information about k. See
...
int
uECC_verify_impl(const uint8_t p_publicKey[uECC_BYTES * 2],
                 const uint8_t p_hash[uECC_BYTES],
                 const uint8_t p_signature[uECC_BYTES * 2])
{
  ...
  if (vli_cmp(curve_n, r) != 1 ||
      vli_cmp(curve_n, s) != 1) { /* r, s must be < n. */
    return 0;
  }
#endif
  ...
  for (i = l_numBits - 2; i >= 0; --i) {
    ...
    uECC_word_t l_index =
      (!!vli_testBit(u1, i)) | ((!!vli_testBit(u2, i)) << 1);
