                             ...
                             const char *econame)
{
  ...
  if (v == NULL) {
     ...
     * (g_vod_mapping_list.next_index - 1), and insert new VOD mapping entry
     * after `v`.
     *
     * After that, continue to increase `next_index` until no `v->index`
     * matching `next_index` is found. After that, next_index will be updated
     * And therefore,`g_vod_mapping_list.vods` list is always sorted in the
     * order of `oc_virtual_device_t.index`...
     */
    while (v != NULL) {
      if ((g_vod_mapping_list.next_index == g_reset_index) ||
        ...
        if (g_vod_mapping_list.next_index == g_reset_index) {
          /*
           * if `next_index` points the first node of
           * `oc_vod_mapping_list.vods`, (v->index ==
           * (g_vod_mapping_list.next_index - 1)) can't be satisfied ever...
           * because there is no VOD mapping entry pointing Device before the
           * first VOD.
           *
           * therefore, insert new VOD mapping entry as the the first item in
          ...
          /*
           * if `next_index` points the middle node of
           * `oc_vod_mapping_list.vods`, finds `v` whose index is
           * (g_vod_mapping_list.next_index - 1). and insert new VOD mapping
           * entry after `v`.
           */
        ...
        /*
         * continue walking the vods mapping entry list till an open next_index
         * is found
         *
        ...
        while (v != NULL) {
          if (v->next != NULL &&
              v->next->index == g_vod_mapping_list.next_index) {
            g_vod_mapping_list.next_index++;
          }
          v = v->next;
        }
