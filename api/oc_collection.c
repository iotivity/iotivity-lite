...
oc_collection_free(oc_collection_t *collection)
{
  if (collection != NULL) {
    ...
    oc_collection_delete_all_links((oc_resource_t *)collection);
...
void
oc_collection_delete_link(oc_resource_t *collection, oc_link_t *link)
{
  if (collection && link) {
    oc_collection_t *c = (oc_collection_t *)collection;
                             ...
                             oc_interface_mask_t interface)
{
  ...
  case OC_IF_BASELINE: {
    ...
    if (oc_string_len(collection->name) > 0) {
      oc_rep_set_text_string(root, n, oc_string(collection->name));
    }
