 ...
 * `get_ns_instance()`) user-defined data structure for `notification selector`
 ...
 * selector` from existing user-defined data structure (`oc_ns_t`)
 *
 * @param resource
...
/*
 * @brief				function used to RETRIEVE `Notification Selector + Push Proxy`
...
/*
 * @brief				function used to UPDATE `Notification Selector + Push Proxy`
 * Resource which is autogenerated through `oic.if.crete` interface
 * @param request
 * @param iface_mask
 * @param user_data
...
/*
 *
 * @brief callback for getting & creating new `Notification Selector + Push
 * Proxy` Resource instance
 *
 */
 ...
 */
static int
_get_obj_array_len(oc_rep_t *obj_list)
{
  int n = 0;
...
/*
 * @brief				create app resource and `oc_pushd_rsc_t` corresponding to pushed
 * resource
 *
 ...
 * 								and app resource pointed by `receiveruri` of each receivre
 * object in the array
 *
