...
{
    int ret;
    ret = oc_storage_config(STORAGE_CONFIG);
    EXPECT_EQ(0, ret);
}
...
{
    int ret;
    ret = oc_storage_config("NEGATIVE_TEST_CASE_FOR_STORAGE_CONFIG_NEGATIVE_TEST_CASE_FOR_STORAGE_CONFIG_NEGATIVE");
...
TEST_F(TestStorage, StorageWriteRead_N)
{
    long ret_write;
    long ret_read;
    ret_write = oc_storage_write(STORAGE_CONFIG_N, buf, OC_MAX_APP_DATA_SIZE);
    EXPECT_EQ(-EINVAL, ret_write);
    ret_read = oc_storage_read(STORAGE_CONFIG_N, buf2, OC_MAX_APP_DATA_SIZE);
