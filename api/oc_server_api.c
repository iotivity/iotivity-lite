                          ...
                          oc_status_t response_code)
{
  ...
  while (cur != NULL) {
    ...
    if (cur->observe > 0) {
      uint16_t cur_mid = coap_get_mid();
      coap_separate_resume(response, cur,
                           (uint8_t)oc_status_code(response_code), cur_mid);
      ...
      coap_transaction_t *t = coap_new_transaction(cur_mid, &cur->endpoint);
      if (t) {
        t->message->length = coap_serialize_message(response, t->message);
