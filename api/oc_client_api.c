...
{
  oc_client_cb_t *cb =
    oc_ri_alloc_client_cb(uri, server, OC_DELETE, handler, user_data, qos);
...
{
  oc_client_cb_t *cb =
    oc_ri_alloc_client_cb(uri, server, OC_POST, handler, user_data, qos);
  if (!cb)
    return false;
...
oc_do_ip_discovery(const char *rt, oc_discovery_cb_t handler)
{
  ...
  oc_client_cb_t *cb =
    oc_ri_alloc_client_cb("/oic/res", &handle, OC_GET, handler, NULL, LOW_QOS);
