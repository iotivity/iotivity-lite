...
static oc_es_enrollee_state g_en_state = OC_ES_STATE_EOF;

typedef void (*oc_es_connect_request_cb)(oc_es_connect_request *);
typedef void (*oc_es_wifi_conf_cb)(oc_es_wifi_conf_data *);
typedef void (*oc_es_dev_conf_cb)(oc_es_dev_conf_data *);
typedef void (*oc_es_rsp_conf_cb)(oc_es_rsp_conf_data *);
typedef void (*oc_es_rspcap_conf_cb)(oc_es_rspcap_conf_data *);
...
typedef struct
{
  ...
  struct
  {
    ...
    oc_es_error_code last_err_code;
  } data;
  oc_es_connect_request_cb connect_request_cb;
} oc_es_easysetup_resource_t;

#define es_res_cast(p) (oc_es_easysetup_resource_t *)(p)

typedef struct
{
  es_resource_t base;
  // This structure is synced with oc_es_wifi_conf_data
  struct
  {
    oc_string_t ssid;
    oc_string_t cred;
  ...
  oc_es_wifi_conf_cb wifi_conf_prov_cb;
...
typedef struct
{
  ...
  oc_es_rspcap_conf_cb rspcap_prov_cb;
              ...
              void *user_data)
{
  ...
  oc_es_easysetup_resource_t *es_res =
              es_res_cast(g_enrollee->res[ES_RES_TYPE_EASYSETUP]);

  oc_rep_start_root_object();
  es_rep_set_int(root, ps, es_res->data.state);
...
update_easysetup_resource(oc_request_t *request)
{
  ...
  if (oc_rep_get_int_array(request->request_payload, OC_RSRVD_ES_CONNECT,
    ...
    if (es_res->data.connect_request[0] != ES_CONNECT_NONE &&
      ...
      oc_es_connect_request conn_req;
      memcpy(&conn_req, &es_res->data, sizeof(oc_es_connect_request));
...
{
  oc_es_wifi_conf_resource_t *wifi_res =
...
{
  bool changed = false;
  oc_es_wifi_conf_data wifi_cb_data;
  oc_es_wifi_conf_resource_t *wifi_res =
    wifi_res_cast(g_enrollee->res[ES_RES_TYPE_WIFI_CONF]);
  ...
  // Invoke callback for user defined attributes
  memcpy(&wifi_cb_data, &wifi_res->data, sizeof(oc_es_wifi_conf_data));
...
{
  oc_es_dev_conf_resource_t *dev_res =
...
{
  oc_es_dev_conf_data dev_cb_data;
...
{
  oc_es_rsp_conf_resource_t *rsp_res =
...
{
  oc_es_rsp_conf_data rsp_cb_data;
...
{
  oc_es_rspcap_conf_resource_t *rspcap_res =
...
{
  oc_es_rspcap_conf_data rspcap_cb_data;
...
static oc_es_result_t
init_easysetup_resource(oc_es_connect_request_cb cb)
{
  g_enrollee = (oc_es_enrollee_t *)calloc(1, sizeof(oc_es_enrollee_t));
  MEM_ALLOC_CHECK(g_enrollee);

  oc_es_easysetup_resource_t *es_res = (oc_es_easysetup_resource_t *)calloc(1, sizeof(oc_es_easysetup_resource_t));
  MEM_ALLOC_CHECK(es_res);

  es_res->data.state = OC_ES_STATE_INIT;
  es_res->data.last_err_code = OC_ES_ERRCODE_NO_ERROR;
...
static oc_es_result_t
init_wifi_conf_resource(oc_es_wifi_conf_cb cb)
{
  INPUT_PARAM_NULL_CHECK(cb);

  oc_es_easysetup_resource_t *es_res =  es_res_cast(g_enrollee->res[ES_RES_TYPE_EASYSETUP]);
  oc_es_wifi_conf_resource_t *wifi_res = (oc_es_wifi_conf_resource_t *)calloc(1, sizeof(oc_es_wifi_conf_resource_t));
...
static oc_es_result_t
init_dev_conf_resource(oc_es_dev_conf_cb cb)
{
  INPUT_PARAM_NULL_CHECK(cb);

  oc_es_easysetup_resource_t *es_res = es_res_cast(g_enrollee->res[ES_RES_TYPE_EASYSETUP]);
  oc_es_dev_conf_resource_t *dev_res = (oc_es_dev_conf_resource_t *)calloc(1, sizeof(oc_es_dev_conf_resource_t));
...
static oc_es_result_t
init_rsp_conf_resource(oc_es_rsp_conf_cb cb)
{
  INPUT_PARAM_NULL_CHECK(cb);

  oc_es_easysetup_resource_t *es_res = es_res_cast(g_enrollee->res[ES_RES_TYPE_EASYSETUP]);
  oc_es_rsp_conf_resource_t *rsp_res = (oc_es_rsp_conf_resource_t *)calloc(1, sizeof(oc_es_rsp_conf_resource_t));
...
static oc_es_result_t
init_rspcap_conf_resource(oc_es_rspcap_conf_cb cb)
{
  INPUT_PARAM_NULL_CHECK(cb);

  oc_es_easysetup_resource_t *es_res = es_res_cast(g_enrollee->res[ES_RES_TYPE_EASYSETUP]);
  oc_es_rspcap_conf_resource_t *rspcap_res = (oc_es_rspcap_conf_resource_t *)calloc(1, sizeof(oc_es_rspcap_conf_resource_t));
...
oc_es_result_t
oc_es_init_enrollee(oc_es_resource_mask_t resource_mask,
                 oc_es_provisioning_callbacks callbacks)
{
  oc_es_result_t res = OC_ES_ERROR;

  if (resource_mask == 0 ||
      resource_mask > (OC_ES_WIFICONF_RESOURCE | OC_ES_DEVCONF_RESOURCE |
        OC_ES_RSPCONF_RESOURCE | OC_ES_RSPCAPCONF_RESOURCE)) {
    OC_ERR("Invalid input!");
    goto exit;
  }

  // Create easysetup collection resource
  res = init_easysetup_resource(callbacks.connect_request_cb);
  if (res != OC_ES_OK) {
    goto exit;
  }

  // Create wificonf resource
  if ((resource_mask & OC_ES_WIFICONF_RESOURCE) == OC_ES_WIFICONF_RESOURCE) {
    res = init_wifi_conf_resource(callbacks.wifi_conf_prov_cb);
    if (res != OC_ES_OK) {
      goto exit;
    }
  }
  ...
  if ((resource_mask & OC_ES_DEVCONF_RESOURCE) == OC_ES_DEVCONF_RESOURCE) {
    res = init_dev_conf_resource(callbacks.dev_conf_prov_cb);
    if (res != OC_ES_OK) {
      goto exit;
    }
  }
  ...
  if ((resource_mask & OC_ES_RSPCONF_RESOURCE) == OC_ES_RSPCONF_RESOURCE) {
    res = init_rsp_conf_resource(callbacks.rsp_conf_prov_cb);
    if (res != OC_ES_OK) {
      goto exit;
    }
  }
  ...
  if ((resource_mask & OC_ES_RSPCAPCONF_RESOURCE) == OC_ES_RSPCAPCONF_RESOURCE) {
    res = init_rspcap_conf_resource(callbacks.rspcap_conf_prov_cb);
    if (res != OC_ES_OK) {
      goto exit;
    }
  }  
...
oc_es_result_t oc_es_set_userdata_callbacks(oc_es_read_userdata_cb_t readcb,
	oc_es_write_userdata_cb_t writecb, oc_es_free_userdata_t free_userdata)
{
  if (!g_enrollee) {
...
oc_es_result_t
oc_es_set_device_info(oc_es_device_info *device_info)
{
  if (!g_enrollee) {
    OC_ERR("Enrollee is not initialized!");
    return OC_ES_ERROR;
  ...
  oc_es_wifi_conf_resource_t *wifi_res =
    wifi_res_cast(g_enrollee->res[ES_RES_TYPE_WIFI_CONF]);
  wifi_res->data.supported_freq = (device_info->WiFi).supported_freq;
...
oc_es_result_t
oc_es_set_state(oc_es_enrollee_state es_state)
{
  g_en_state = es_state;
  ...
  oc_es_easysetup_resource_t *es_res =
    es_res_cast(g_enrollee->res[ES_RES_TYPE_EASYSETUP]);
  es_res->data.state = es_state;
  oc_notify_observers(es_res->base.handle);
  return OC_ES_OK;
}

oc_es_enrollee_state
oc_es_get_state(void)
{
  return g_en_state;
}

oc_es_result_t
oc_es_set_error_code(oc_es_error_code err_code)
{
  if (!g_enrollee) {
    OC_ERR("Enrollee is not initialized!");
    return OC_ES_ERROR;
  ...
  if (err_code < OC_ES_ERRCODE_NO_ERROR || err_code > OC_ES_ERRCODE_UNKNOWN) {
  ...
  oc_es_easysetup_resource_t *es_res =
    es_res_cast(g_enrollee->res[ES_RES_TYPE_EASYSETUP]);
  es_res->data.last_err_code = err_code;
  oc_notify_observers(es_res->base.handle);
  return OC_ES_OK;
}
