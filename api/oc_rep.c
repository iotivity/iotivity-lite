...
void oc_rep_print_byte_string(oc_rep_t *rep) {
    assert(rep->type == OC_REP_BYTE_STRING);
    ...
    int output_len = oc_base64_encode((uint8_t *)byte_string, byte_string_size, (uint8_t *)b64_buf, b64_buf_size);
    if (output_len < 0) {
        free(b64_buf);
        return;
    }
...
void oc_rep_print_format(oc_rep_t *rep, int tab_depth, bool pretty_print) {
    while (rep != NULL) {
        ...
        case OC_REP_BOOL:
            ...
            break;
        case OC_REP_BYTE_STRING: ;//hack work around for compiler error
        oc_rep_print_byte_string(rep);
            break;
        case OC_REP_STRING:
