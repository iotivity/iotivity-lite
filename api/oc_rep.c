...
static bool
_is_match_type(oc_rep_t *rep, oc_rep_value_type_t type)
{
  if (rep->type == type)
    return true;

  if (OC_REP_STRING == type && rep->type == OC_REP_BYTE_STRING)
    return true;

  if (OC_REP_ARRAY == type &&
      (OC_REP_OBJECT < rep->type && OC_REP_OBJECT_ARRAY > rep->type))
    return true;

  if (OC_REP_OBJECT == type && rep->type == OC_REP_OBJECT_ARRAY)
    return true;

  OC_WRN("Type is %d", rep->type);
  return false;
}
...
oc_rep_get_value(oc_rep_t *rep, oc_rep_value_type_t type, const char *key,
                 void *value)
{
  if (!rep || !key || !value || !_is_match_type(rep, type)) {
  ...
  while (rep_value != NULL) {
    if (strncmp(key, oc_string(rep_value->name),
