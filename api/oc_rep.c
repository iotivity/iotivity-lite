  ...
  while (rep_value != NULL) {
    if ((oc_string_len(rep_value->name) == strlen(key)) &&
      ...
      case OC_REP_ARRAY:
        ...
        *length = (int)rep_value->value.array.size;
        break;
      case OC_REP_INT_ARRAY:
        *value = oc_int_array(rep_value->value.array);
        *length = (int)oc_int_array_size(rep_value->value.array);
        break;
      case OC_REP_BOOL_ARRAY:
        *value = oc_bool_array(rep_value->value.array);
        *length = (int)oc_bool_array_size(rep_value->value.array);
        break;
      case OC_REP_DOUBLE_ARRAY:
        *value = oc_double_array(rep_value->value.array);
        *length = (int)oc_double_array_size(rep_value->value.array);
        break;
      case OC_REP_BYTE_STRING_ARRAY:
      case OC_REP_STRING_ARRAY:
...
bool
oc_rep_get_byte_string(oc_rep_t *rep, const char *key, oc_string_t *value)
{
  return oc_rep_get_value(rep, OC_REP_BYTE_STRING, key, (void **)&value, NULL);
}

bool
oc_rep_get_string(oc_rep_t *rep, const char *key, oc_string_t *value)
{
  return oc_rep_get_value(rep, OC_REP_STRING, key, (void **)&value, NULL);
}

bool
oc_rep_get_array(oc_rep_t *rep, const char *key, oc_array_t *value, int *length)
{
  if (!length)
    return false;
  return oc_rep_get_value(rep, OC_REP_ARRAY, key, (void **)&value, length);
}

bool
oc_rep_get_int_array(oc_rep_t *rep, const char *key, int *value, int *length)
{
  if (!length)
    return false;
  return oc_rep_get_value(rep, OC_REP_INT_ARRAY, key, (void **)&value, length);
}

bool
oc_rep_get_bool_array(oc_rep_t *rep, const char *key, bool *value, int *length)
{
  if (!length)
    return false;
  return oc_rep_get_value(rep, OC_REP_BOOL_ARRAY, key, (void **)&value, length);
}

bool
oc_rep_get_double_array(oc_rep_t *rep, const char *key, double *value, int *length)
{
  if (!length)
    return false;
  return oc_rep_get_value(rep, OC_REP_DOUBLE_ARRAY, key, (void **)&value, length);
}

bool
oc_rep_get_byte_string_array(oc_rep_t *rep, const char *key, oc_string_array_t *value, int *length)
{
  if (!length)
    return false;
  return oc_rep_get_value(rep, OC_REP_BYTE_STRING_ARRAY, key, (void **)&value, length);
}

bool
oc_rep_get_string_array(oc_rep_t *rep, const char *key, oc_string_array_t *value, int *length)
{
  if (!length)
    return false;
  return oc_rep_get_value(rep, OC_REP_STRING_ARRAY, key, (void **)&value, length);
}
