  ...
  while (rep_value != NULL) {
    if ((oc_string_len(rep_value->name) == strlen(key)) &&
      ...
      case OC_REP_ARRAY:
        ...
        *size = (int)rep_value->value.array.size;
        break;
      case OC_REP_INT_ARRAY:
...
bool
oc_rep_get_array(oc_rep_t *rep, const char *key, oc_array_t *value, int *size)
{
  if (!size)
    return false;
  return oc_rep_get_value(rep, OC_REP_ARRAY, key, (void **)&value, size);
}
