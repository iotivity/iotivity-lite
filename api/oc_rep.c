 ...
 * buffer so it the addition of each string looks like concatenation means moving
 * the buf pointer forward and reducing the buf_size after every function that
 * adds to the buffer.
 *
...
#define OC_JSON_UPDATE_BUFFER_AND_TOTAL \
  ...
  if (num_char_printed < buf_size && buf != NULL) \
    { buf += num_char_printed; buf_size -= num_char_printed; } \
  else \
    {buf += buf_size; buf_size = 0;} \
} while(0);
 ...
 * white-space.
 */
size_t oc_rep_to_json_tab(char*buf, size_t buf_size, int tab_depth) {
  size_t num_char_printed = 0;
  size_t total_char_printed = 0;
  for (int i = 0; i < tab_depth; i++) {
    num_char_printed = snprintf(buf, buf_size,"%s", OC_PRETTY_PRINT_TAB_CHARACTER);
...
size_t oc_rep_to_json_base64_encoded_byte_string(char *buf, size_t buf_size, char *byte_str, size_t byte_str_size) {
  ...
  num_char_printed = snprintf(buf, buf_size, "\"%s\"", b64_buf);
  OC_JSON_UPDATE_BUFFER_AND_TOTAL;
  free(b64_buf);
  ...
  {
    int output_len = oc_base64_encode((uint8_t *)byte_str, byte_str_size, (uint8_t *)buf, b64_buf_size);
    num_char_printed = output_len;
    OC_JSON_UPDATE_BUFFER_AND_TOTAL;
  } else {
    int output_len = oc_base64_encode((uint8_t *)byte_str, byte_str_size, (uint8_t *)buf, buf_size);
    num_char_printed = buf_size;
 ...
 *
 * oc_rep_to_json_format will take any oc_rep_t and print out the json equivalent
 ...
 */
size_t oc_rep_to_json_format(oc_rep_t *rep, char *buf, size_t buf_size, int tab_depth, bool pretty_print) {
  (void) buf;
  (void) buf_size;
  size_t num_char_printed = 0;
  size_t total_char_printed = 0;
  while (rep != NULL) {
    ...
    num_char_printed = (pretty_print) ? snprintf(buf, buf_size, "\"%s\" : ", oc_string(rep->name))
    ...
    case OC_REP_INT:
    {
      num_char_printed = snprintf(buf, buf_size, "%"PRId64 , rep->value.integer);
      OC_JSON_UPDATE_BUFFER_AND_TOTAL;
      break;
    }
    case OC_REP_DOUBLE:
    ...
    case OC_REP_BOOL:
    {
      num_char_printed = snprintf(buf, buf_size, "%s", (rep->value.boolean) ? "true" : "false");
      OC_JSON_UPDATE_BUFFER_AND_TOTAL;
      break;
    }
    case OC_REP_BYTE_STRING:
    {
      ...
      num_char_printed = oc_rep_to_json_base64_encoded_byte_string(buf, buf_size, byte_string, byte_string_size);
      OC_JSON_UPDATE_BUFFER_AND_TOTAL;
      break;
    }
    case OC_REP_STRING:
    {
      num_char_printed = snprintf(buf, buf_size, "\"%s\"", oc_string(rep->value.string));
      OC_JSON_UPDATE_BUFFER_AND_TOTAL;
      break;
    }
    case OC_REP_OBJECT:
    {
      ...
      OC_JSON_UPDATE_BUFFER_AND_TOTAL;
      num_char_printed = oc_rep_to_json_format(rep->value.object, buf, buf_size, tab_depth + 1, pretty_print);
      OC_JSON_UPDATE_BUFFER_AND_TOTAL;
      if (pretty_print){
        num_char_printed = oc_rep_to_json_tab(buf, buf_size, tab_depth + 1);
        OC_JSON_UPDATE_BUFFER_AND_TOTAL;
    ...
    case OC_REP_BOOL_ARRAY:
    {
      ...
      for (size_t i = 0; i < bool_array_size; i++) {
        num_char_printed = snprintf(buf, buf_size, "%s", (bool_array[i]) ? "true" : "false");
        OC_JSON_UPDATE_BUFFER_AND_TOTAL;
    ...
    case OC_REP_STRING_ARRAY:
    {
      ...
      for (size_t i = 0; i < str_array_size; i++) {
        ...
        num_char_printed = snprintf(buf, buf_size, "\"%s\"", oc_string_array_get_item(str_array, i));
        OC_JSON_UPDATE_BUFFER_AND_TOTAL;
    ...
    case OC_REP_OBJECT_ARRAY:
    {
      ...
      do {
        ...
        num_char_printed = (pretty_print) ? snprintf(buf, buf_size, "{\n")
                                          : snprintf(buf, buf_size, "{");
        OC_JSON_UPDATE_BUFFER_AND_TOTAL;
        num_char_printed = oc_rep_to_json_format(rep_item, buf, buf_size, tab_depth + 2, pretty_print);
        OC_JSON_UPDATE_BUFFER_AND_TOTAL;
