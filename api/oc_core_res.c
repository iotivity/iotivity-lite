                       ...
                       void *data)
{
  ...
  char di[MAX_UUID_LENGTH], piid[MAX_UUID_LENGTH];
  oc_uuid_to_str(&oc_device_info[device].di, di, MAX_UUID_LENGTH);
  if (request->origin && request->origin->version != OIC_VER_1_1_0) {
    oc_uuid_to_str(&oc_device_info[device].piid, piid, MAX_UUID_LENGTH);
  }
                         ...
                         void *data)
{
  ...
  char pi[MAX_UUID_LENGTH];
  oc_uuid_to_str(&oc_platform_info.pi, pi, MAX_UUID_LENGTH);
