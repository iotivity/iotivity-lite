...
static void
oc_core_con_handler(oc_request_t *request, oc_interface_mask_t interface,
                    void *data)
{
    (void)data;
    int device = request->resource->device;
...
next_di = NULL;
  ...
  oc_core_populate_resource(
      ocf_d + 1, device_count, con_uri, OC_IF_R | OC_IF_RW, OC_IF_R,
      OC_DISCOVERABLE | OC_OBSERVABLE, oc_core_con_handler, 0, 0, 0, 1, "oic.wk.con");
