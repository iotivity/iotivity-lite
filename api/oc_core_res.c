                         ...
                         void *data)
{
  ...
  while (rep != NULL) {
    if (strcmp(oc_string(rep->name), "n") == 0) {
      ...
      oc_rep_end_root_object();
      if (data) {
        ((oc_con_write_cb_t)data)(device, rep);
      }
      oc_send_response(request, OC_STATUS_CHANGED);
      /* notify_observers is automatically triggered in
         oc_ri_invoke_coap_entity_handler() for oic.wk.con,
         but since we are also changing the n of oic.wk.d, we
         need to notify this separately */
      oc_notify_observers(
          oc_core_get_resource_by_index(NUM_OC_CORE_RESOURCES - 1 + device));
      return;
    }
    rep = rep->next;
  }
