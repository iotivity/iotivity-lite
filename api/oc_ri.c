 ...
 */
  ...
  if (cur_resource && !bad_request) {
    ...
    if (!oc_sec_check_acl(method, cur_resource, endpoint)) {
      ...
      char aux_arr[5][LINE_WIDTH];
      memset(aux_arr, 0, sizeof(aux_arr));
      char* aux[] = {aux_arr[0], aux_arr[1], aux_arr[2], aux_arr[3], aux_arr[4]};
      size_t idx = 0;
      SNPRINTFipaddr(aux[idx++], LINE_WIDTH, *endpoint);
      oc_tls_peer_t *peer = oc_tls_get_peer(endpoint);
      if (peer) {
        oc_uuid_to_str(&peer->uuid, aux[idx++], LINE_WIDTH);
      }
      memcpy(aux[idx++], oc_string(cur_resource->uri), oc_string_len(cur_resource->uri));
      ...
      snprintf(aux[idx++], LINE_WIDTH, "attempt to %s, when device is in %s\n", method_str_val[method], state_str_val[state]);
#ifndef OC_PKI
      ...
      if (peer) {
        ...
        for (oc_sec_cred_t *rc = oc_sec_get_roles(peer); rc && pos < LINE_WIDTH; rc = rc->next) {
          pos += snprintf(aux[idx]+pos, LINE_WIDTH-pos-1, "%s ", oc_string(rc->role.role));
        }
        if (pos != 0) {
            idx++;
        }
...
#endif /* OC_PKI */
      oc_audit_log("EAC-1", "Access Denied", 0x01, 2, (const char **)aux, idx);
    } else
#endif /* OC_SECURITY */
    {
