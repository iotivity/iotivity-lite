                      ...
                      st_cloud_access_cb_t cb)
{
  ...
  context->callback = cb;
  context->cloud_access_status = cloud_info->cloudinfo.status;
...
get_cloud_access_status(int device_index)
{
  ...
  return (context->cloud_access_status & CLOUD_ACCESS_FINISH);
}
...
callback_handler(void *data)
{
  ...
  st_cloud_access_status_t status = CLOUD_ACCESS_INITIALIZE;
  if (context->cloud_access_status & CLOUD_ACCESS_FAIL)
    status = CLOUD_ACCESS_FAIL;
  else if (context->cloud_access_status & CLOUD_ACCESS_FINISH)
    status = CLOUD_ACCESS_FINISH;
...
cloud_start_process(st_cloud_context_t *context)
{
  ...
  context->cloud_access_status &=
    ~(CLOUD_ACCESS_SIGNED_IN & CLOUD_ACCESS_FINISH);
...
sign_in_handler(oc_client_response_t *data)
{
  ...
  if (data->code == OC_STATUS_CHANGED) {
    ...
    if (context->cloud_access_status & CLOUD_ACCESS_PUBLISHED) {
