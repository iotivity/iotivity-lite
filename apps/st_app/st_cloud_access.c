...
{
  st_print_log("%s in ci_server %s\n", __FUNCTION__, ci_server);
  oc_string_t dns_str;
  ...
  if (oc_string_to_endpoint(&dns_str, &ep, NULL) != 0) {
    ...
    st_print_log("%s error in getting conn server endpoint!\n", __FUNCTION__);
    return -1;
  }
  oc_free_string(&dns_str);
  ...
  if (ci_server) {
    ...
    if (oc_string_to_endpoint(&dns_str, &ep, NULL) != 0) {
      oc_free_string(&dns_str);
      st_print_log("%s error in getting ci server endpoint!\n", __FUNCTION__);
      return -1;
    }
    oc_free_string(&dns_str);
