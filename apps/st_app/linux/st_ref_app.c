    ...
    for (int device_num = 0; device_num < device_cnt; device_num++) {
        ...
        if (resources != NULL) {
            ...
            if (collection != NULL) {
                ...
                st_print_log("[ST_APP] [DEVICE] Resources for Collection Cnt : %d\n", node->col_cnt);
                ...
                for (int iter = 0; iter < node->col_cnt; iter++) {
                    ...
                    if (res->type != NULL) {
                        ...
                        st_print_log("[ST_APP] [COLLECTION] collection[0].uri : %s\n", (node->collection[iter].uri));
                        ...
                        if (types) {
                            ...
                            for (int typeiter = 0; typeiter < type_cnt; typeiter++) {
                                ...
                                st_print_log("[ST_APP] [COLLECTION] collection[iter].resource_types[typeiter] : %s\n", (node->collection[iter].resource_types[typeiter]));
                            }
                        } else {
                        ...
                        st_print_log("[ST_APP] [COLLECTION] collection[iter].policy : %d\n", (node->collection[iter].policy));
                        ...
                        if (links != NULL) {
                            ...
                            st_print_log("[ST_APP] [COLLECTION] collection[iter].link_cnt : %d\n", (node->collection[iter].link_cnt));
                            for (int linkiter = 0; linkiter < linkCnt; linkiter++) {
                                ...
                                if (uri != NULL) {
                                    ...
                                    st_print_log("[ST_APP] [COLLECTION] link_resource->uri : %s\n", (link_resource->uri));
                                }
                ...
                st_print_log("[ST_APP] [COLLECTION] URI. : %s\n", node->collection[0].uri);
                st_print_log("[ST_APP] [COLLECTION] interface_type. : %s\n", node->collection[0].interface_types[0]);
                st_print_log("[ST_APP] [COLLECTION] resource_types. : %s\n", node->collection[0].resource_types[0]);
                st_print_log("[ST_APP] [COLLECTION] if_cnt. : %d\n", node->collection[0].if_cnt);
                st_print_log("[ST_APP] [COLLECTION] rt_cnt. : %d\n", node->collection[0].rt_cnt);
                st_print_log("[ST_APP] [COLLECTION] link_cnt. : %d\n", node->collection[0].link_cnt);
                st_print_log("[ST_APP] [COLLECTION] policy. : %d\n", node->collection[0].policy);
            } else {
...
main(void)
{
  ...
  int line_failed;
  if ((line_failed = st_manager_json_parse()) != 0){
    st_print_log("[ST_APP] st_manager_json_parse failed at line = %d.\n",line_failed);
  }
