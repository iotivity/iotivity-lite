                      ...
                      st_cloud_access_cb_t cb)
{
  ...
  if (strncmp(cloud_info->auth_provider, AUTH_PROVIDER_GITHUB,
    ...
    if (st_cloud_info) {
      es_set_state(ES_STATE_REGISTERING_TO_CLOUD);
      oc_new_string(&context->uid, st_cloud_info->clientID,
                    strlen(st_cloud_info->clientID));
      oc_sign_up(&context->cloud_ep, cloud_info->auth_provider,
                 st_cloud_info->clientID, cloud_info->access_token,
                 device_index, sign_up_handler, context);
    }
  } else {
    goto errors;
  }
