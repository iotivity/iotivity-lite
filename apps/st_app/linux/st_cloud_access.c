...
sign_up_process(st_cloud_context_t *context)
{
  if (strncmp(oc_string(context->auth_provider), AUTH_PROVIDER_GITHUB,
    ...
    oc_sign_up_with_auth(&context->cloud_ep, oc_string(context->auth_provider),
                         oc_string(context->auth_code), 0, sign_up_handler,
                         context);
  } else if (strncmp(oc_string(context->auth_provider), AUTH_PROVIDER_STCLOUD,
                     strlen(AUTH_PROVIDER_STCLOUD)) == 0) {
    char device_id[100];
    oc_uuid_to_str(oc_core_get_device_id(context->device_index), device_id,
                   MAX_UUID_LENGTH);
    es_set_state(ES_STATE_REGISTERING_TO_CLOUD);
    oc_sign_up(&context->cloud_ep, oc_string(context->auth_provider), device_id,
               oc_string(context->access_token), context->device_index,
               sign_up_handler, context);
  } else {
    return false;
  }
  return true;
...
sign_up_handler(oc_client_response_t *data)
{
  ...
  if (data->code == OC_STATUS_CHANGED) {
    ...
    es_set_state(ES_STATE_FAILED_TO_REGISTER_TO_CLOUD);
    oc_set_delayed_callback(context, &callback_handler, 0);
    oc_set_delayed_callback(context, &re_sign_up_handler, RETRY_INTERVAL);
  }
}
