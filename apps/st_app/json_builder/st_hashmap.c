...
static void rehash(struct hashmap_s *hm)
{
	...
	hm->table = (h_entry_s *) oc_mem_calloc(sizeof(h_entry_s), hm->size);
	hm->count = 0;
...
{
	struct hashmap_s *hm = (struct hashmap_s *)oc_mem_malloc(sizeof(struct hashmap_s));
	...
	hm->table = (h_entry_s *) oc_mem_calloc(sizeof(h_entry_s), startsize);
	hm->size = startsize;
	hm->count = 0;
...
unsigned long* hashmap_get_keyset(struct hashmap_s *hash)
{
	...
	if (hash->count) {
		...
		keyset = oc_mem_malloc(sizeof(unsigned long) * hash->count);
...
void hashmap_delete(struct hashmap_s *hash, delete_function del_func)
{
    if (hash == NULL) {
        return;
    }
    if(del_func != NULL){
        for (int i = 0; i < hash->size; i++) {
            if (hash->table[i].flags & ACTIVE && hash->table[i].data) {
                del_func(hash->table[i].data);
            }
        }
    }
    oc_mem_free(hash->table);
    oc_mem_free(hash);
}
