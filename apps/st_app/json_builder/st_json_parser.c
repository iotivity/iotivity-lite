...
static int resource_type_cnt = 0;
...
static void delete_resource_type(void *resource_type)
{
    oc_mem_free(resource_type);
}

static void delete_device(void *data)
{
    st_device_s *device = data;
    if (device != NULL) {
...
static void copy_if_valid(char **dst, cJSON *src){
    if(!dst || !src)
        return;
    int src_len = strlen(src->valuestring) + 1; // +1 for NULL termination
    *dst = (char *) oc_mem_malloc(sizeof(char) * src_len);
    if(*dst == NULL) {
        st_print_log("[ST_APP] Unable to allocate memory!\n");
        return;
    }
    oc_assert(*dst != NULL);
    strncpy(*dst, src->valuestring, src_len);
}
    ...
    for (int device_num = 0; device_num < device_cnt; device_num++) {
        ...
        if (specification != NULL) {
            ...
            if (spec_device != NULL) {
                ...
                if (device_type != NULL) {
                    copy_if_valid(&node->type, device_type);
                }

                if (device_name != NULL) {
                    copy_if_valid(&node->name, device_name);
                }

                if (spec_version != NULL) {
                    copy_if_valid(&node->spec_version, spec_version);
                }

                if (data_model_version != NULL) {
                    copy_if_valid(&node->data_model_version, data_model_version);
                }
            }
            ...
            if (spec_platform != NULL) {
                ...
                if (manufacturer_name != NULL) {
                    ...
                    copy_if_valid(&node->manufacturer_name, manufacturer_name);
                }
                if (manufacturer_url != NULL) {
                    copy_if_valid(&node->manufacturer_url, manufacturer_url);
                }
                if (manufacturing_date != NULL) {
                    copy_if_valid(&node->manufacturing_date, manufacturing_date);
                }
                if (model_number != NULL) {
                    copy_if_valid(&node->model_num, model_number);
                }
                if (platform_version != NULL) {
                    copy_if_valid(&node->ver_p, platform_version);
                }
                if (os_version != NULL) {
                    copy_if_valid(&node->ver_os, os_version);
                }
                if (hardware_version != NULL) {
                    copy_if_valid(&node->ver_hw, hardware_version);
                }
                if (firmware_version != NULL) {
                    copy_if_valid(&node->ver_fw, firmware_version);
                }
                if (vendor_id != NULL) {
                    copy_if_valid(&node->vender_id, vendor_id);
                }
            }
        }
        st_print_log("[ST_APP] [DEVICE] No. : %d", (node->no));
        st_print_log(" type : %s", (node->type));
        st_print_log(" name : %s", (node->name));
        st_print_log(" mf_name : %s", (node->manufacturer_name));
        st_print_log(" mf_url : %s", (node->manufacturer_url));
        st_print_log(" mf_date : %s", (node->manufacturing_date));
        st_print_log(" model num : %s", (node->model_num));
        st_print_log(" plat. ver : %s", (node->ver_p));
        st_print_log(" os version : %s", (node->ver_os));
        st_print_log(" hw version : %s", (node->ver_hw));
        st_print_log(" fw version : %s", (node->ver_fw));
        st_print_log(" vender id : %s\n", (node->vender_id));
...
void st_manager_json_deinitialize(void){
    hashmap_delete(g_device_hmap,delete_device);
    hashmap_delete(g_resource_type_hmap,delete_resource_type);
}

st_device_s *st_manager_json_get_device(int device_index){
    oc_assert(g_device_hmap != NULL);
    return hashmap_get(g_device_hmap,(unsigned long)device_index);
}
const char *st_manager_json_get_setup_id(void){
    return g_setup_id;
}
