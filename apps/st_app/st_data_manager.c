...
st_decode_resource_types(oc_rep_t *rsc_type_rep)
{
  ...
  if (oc_rep_get_object_array(rsc_type_rep, ST_RSC_TYPES_PROPS_KEY,
    ...
    while (cur) {
      ...
      if (oc_rep_get_string(cur, ST_PROPS_KEY_KEY, &value, &size)) {
        st_string_check_new(&property->key, value, size);
      }

      int type;
      if (oc_rep_get_int(cur, ST_PROPS_TYPE_KEY, &type)) {
        property->type = type;
      }

      bool mandatory;
      if (oc_rep_get_bool(cur, ST_PROPS_MANDATORY_KEY, &mandatory)) {
        property->mandatory = mandatory;
      }

      int rw;
      if (oc_rep_get_int(cur, ST_PROPS_RW_KEY, &rw)) {
        property->rw = rw;
      }
