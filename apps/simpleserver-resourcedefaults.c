...
int g_binaryswitch_both_storage_status = 0;  /* 0=no storage, 1=startup, 2=startup.revert */
static char *g_binaryswitch_both_RESOURCE_PROPERTY_NAME_value = "value"; /* the name for the attribute */
bool g_binaryswitch_both_value = false; /* current value of property "value" The status of the switch. */
/* global property variables for path: "/binaryswitch_revert" */
int g_binaryswitch_revert_storage_status = 0;  /* 0=no storage, 1=startup, 2=startup.revert */
static char *g_binaryswitch_revert_RESOURCE_PROPERTY_NAME_value = "value"; /* the name for the attribute */
bool g_binaryswitch_revert_value = false; /* current value of property "value" The status of the switch. */
/* global property variables for path: "/binaryswitch_startup" */
int g_binaryswitch_startup_storage_status = 0;  /* 0=no storage, 1=startup, 2=startup.revert */
static char *g_binaryswitch_startup_RESOURCE_PROPERTY_NAME_value = "value"; /* the name for the attribute */
bool g_binaryswitch_startup_value = false; /* current value of property "value" The status of the switch. *//* registration data variables for the resources */

/* global resource variables for path: /binaryswitch_both */
static char *g_binaryswitch_both_RESOURCE_ENDPOINT = "/binaryswitch_both"; /* used path for this resource */
static char *g_binaryswitch_both_RESOURCE_TYPE[MAX_STRING] = {"oic.r.switch.binary"}; /* rt value (as an array) */
int g_binaryswitch_both_nr_resource_types = 1;
/* global resource variables for path: /binaryswitch_revert */
static char *g_binaryswitch_revert_RESOURCE_ENDPOINT = "/binaryswitch_revert"; /* used path for this resource */
static char *g_binaryswitch_revert_RESOURCE_TYPE[MAX_STRING] = {"oic.r.switch.binary"}; /* rt value (as an array) */
int g_binaryswitch_revert_nr_resource_types = 1;
/* global resource variables for path: /binaryswitch_startup */
static char *g_binaryswitch_startup_RESOURCE_ENDPOINT = "/binaryswitch_startup"; /* used path for this resource */
static char *g_binaryswitch_startup_RESOURCE_TYPE[MAX_STRING] = {"oic.r.switch.binary"}; /* rt value (as an array) */
...
app_init(void)
{
  ...
  fp = fopen("c:/users/m.trayer/OCF/ResourceDefaults/server_introspection.cbor", "rb");
  if (fp) {
    fseek(fp, 0, SEEK_END);
...
*/
static void
get_binaryswitch_both(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
  ...
  case OC_IF_BASELINE:
    ...
    PRINT("   %s : %s\n", g_binaryswitch_both_RESOURCE_PROPERTY_NAME_value,  btoa(g_binaryswitch_both_value));
    break;
  case OC_IF_A:
  ...
  case OC_IF_STARTUP:
   if ( g_binaryswitch_both_storage_status !=  1)
   {
    ...
    /* property (boolean) 'value' */
    {
     bool temp_value;
     long temp_size;
     temp_size = oc_storage_read("g_binaryswitch_both_value", (uint8_t*)&temp_value, sizeof(temp_value));
      oc_rep_set_boolean(root, value, temp_value);
      PRINT("   (startup) %s : %s (%ld)\n", g_binaryswitch_both_RESOURCE_PROPERTY_NAME_value,  btoa(temp_value), temp_size);
    }
    break;
...
*/
static void
get_binaryswitch_revert(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
  (void)user_data;  /* variable not used */
  /* TODO: SENSOR add here the code to talk to the HW if one implements a sensor.
     the call to the HW needs to fill in the global variable before it returns to this function here.
  ...
  case OC_IF_BASELINE:
    ...
    PRINT("   %s : %s\n", g_binaryswitch_revert_RESOURCE_PROPERTY_NAME_value,  btoa(g_binaryswitch_revert_value));
    break;
  case OC_IF_A:
  ...
  case OC_IF_STARTUP_REVERT:
   ...
   oc_status_code = OC_STATUS_NOT_MODIFIED;
    /* No payload is sent for a RETRIEVE in Revert
    {
     bool temp_value;
     long temp_size;
     temp_size = oc_storage_read("g_binaryswitch_revert_value", (uint8_t*)&temp_value, sizeof(temp_value));
     // oc_rep_set_boolean(root, value, temp_value);
     PRINT("   (startup) %s : %s (%ld)\n", g_binaryswitch_revert_RESOURCE_PROPERTY_NAME_value,  btoa(temp_value), temp_size);
    }
	*/
    break;
  default:
...
*/
static void
get_binaryswitch_startup(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
  (void)user_data;  /* variable not used */
  /* TODO: SENSOR add here the code to talk to the HW if one implements a sensor.
     the call to the HW needs to fill in the global variable before it returns to this function here.
  ...
  case OC_IF_BASELINE:
    ...
    PRINT("   %s : %s\n", g_binaryswitch_startup_RESOURCE_PROPERTY_NAME_value,  btoa(g_binaryswitch_startup_value));
    break;
  case OC_IF_A:
  ...
  case OC_IF_STARTUP:
   if ( g_binaryswitch_startup_storage_status !=  1)
   {
    ...
    /* property (boolean) 'value' */
    {
     bool temp_value;
     long temp_size;
     temp_size = oc_storage_read("g_binaryswitch_startup_value", (uint8_t*)&temp_value, sizeof(temp_value));
      oc_rep_set_boolean(root, value, temp_value);
      PRINT("   (startup) %s : %s (%ld)\n", g_binaryswitch_startup_RESOURCE_PROPERTY_NAME_value,  btoa(temp_value), temp_size);
    }
    break;
  default:
    break;
...
*/
static void
post_binaryswitch_both(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
  (void)interfaces;
  (void)user_data;
  bool error_state = false;
  ...
  while (rep != NULL) {
    if (strcmp ( oc_string(rep->name), g_binaryswitch_both_RESOURCE_PROPERTY_NAME_value) == 0) {
      var_in_request = true;
    }
    rep = rep->next;
  }
  ...
  if (error_state == false)
  {
    ...
    case OC_IF_STARTUP: {
      ...
      oc_storage_write("g_binaryswitch_both_storage_status", (uint8_t*)&g_binaryswitch_both_storage_status, sizeof(g_binaryswitch_both_storage_status));
      ...
      while (rep != NULL) {
        ...
        if (strcmp ( oc_string(rep->name), g_binaryswitch_both_RESOURCE_PROPERTY_NAME_value)== 0) {
          /* write storage "value" */
          long temp_size;
          temp_size = oc_storage_write("g_binaryswitch_both_value", (uint8_t*)&rep->value.boolean, sizeof(g_binaryswitch_both_value));
  ...
  default: {  
    ...
    oc_rep_start_root_object();
    /*oc_process_baseline_interface(request->resource); */
    PRINT("   %s : %s", g_binaryswitch_both_RESOURCE_PROPERTY_NAME_value, btoa(g_binaryswitch_both_value));
    oc_rep_set_boolean(root, value, g_binaryswitch_both_value);
    //oc_storage_write("g_binaryswitch_both_value", (uint8_t*)&g_binaryswitch_both_value, sizeof(g_binaryswitch_both_value));
...
*/
static void
post_binaryswitch_revert(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
  (void)interfaces;
  (void)user_data;
  bool error_state = false;
  ...
  rep = request->request_payload;
  while (rep != NULL) {
    if (strcmp ( oc_string(rep->name), g_binaryswitch_revert_RESOURCE_PROPERTY_NAME_value) == 0) {
      var_in_request = true;
    }
    rep = rep->next;
  }
  ...
  case OC_IF_STARTUP_REVERT: {
      ...
      oc_storage_write("g_binaryswitch_revert_storage_status", (uint8_t*)&g_binaryswitch_revert_storage_status, sizeof(g_binaryswitch_revert_storage_status));
      /* loop over all the properties in the input document */
      oc_rep_t *rep = request->request_payload;
      while (rep != NULL) {
        PRINT("key: (assign startup) %s \n", oc_string(rep->name));
        ...
        if (strcmp ( oc_string(rep->name), g_binaryswitch_revert_RESOURCE_PROPERTY_NAME_value)== 0) {
          /* write storage "value" */
          long temp_size;
          temp_size = oc_storage_write("g_binaryswitch_revert_value", (uint8_t*)&rep->value.boolean, sizeof(g_binaryswitch_revert_value));
          PRINT ("  storage (startup.revert)  property 'value' : %s (%ld)\n", btoa(rep->value.boolean), temp_size);
        }
        rep = rep->next;
    }
    ...
    default: {  
      ...
      oc_rep_start_root_object();
      /*oc_process_baseline_interface(request->resource); */
      PRINT("   %s : %s", g_binaryswitch_revert_RESOURCE_PROPERTY_NAME_value, btoa(g_binaryswitch_revert_value));
      oc_rep_set_boolean(root, value, g_binaryswitch_revert_value);
      oc_storage_write("g_binaryswitch_revert_value", (uint8_t*)&g_binaryswitch_revert_value, sizeof(g_binaryswitch_revert_value));
...
*/
static void
post_binaryswitch_startup(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
  (void)interfaces;
  (void)user_data;
  bool error_state = false;
  ...
  rep = request->request_payload;
  while (rep != NULL) {
    if (strcmp ( oc_string(rep->name), g_binaryswitch_startup_RESOURCE_PROPERTY_NAME_value) == 0) {
      var_in_request = true;
    }
    rep = rep->next;
  }
  ...
  if (error_state == false)
  {
    ...
    case OC_IF_STARTUP: {
      ...
      oc_storage_write("g_binaryswitch_startup_storage_status", (uint8_t*)&g_binaryswitch_startup_storage_status, sizeof(g_binaryswitch_startup_storage_status));
      /* loop over all the properties in the input document */
      oc_rep_t *rep = request->request_payload;
      while (rep != NULL) {
        PRINT("key: (assign startup) %s \n", oc_string(rep->name));
        ...
        if (strcmp ( oc_string(rep->name), g_binaryswitch_startup_RESOURCE_PROPERTY_NAME_value)== 0) {
          /* write storage "value" */
          long temp_size;
          temp_size = oc_storage_write("g_binaryswitch_startup_value", (uint8_t*)&rep->value.boolean, sizeof(g_binaryswitch_startup_value));
          PRINT ("  storage (startup.revert)  property 'value' : %s (%ld)\n", btoa(rep->value.boolean), temp_size);
        }
        rep = rep->next;
    }
    ...
    default: {
      ...
      oc_rep_start_root_object();
      /*oc_process_baseline_interface(request->resource); */
      PRINT("   %s : %s", g_binaryswitch_startup_RESOURCE_PROPERTY_NAME_value, btoa(g_binaryswitch_startup_value));
      oc_rep_set_boolean(root, value, g_binaryswitch_startup_value);
...
register_resources(void)
{
  ...
  res_binaryswitch_both = oc_new_resource(NULL, g_binaryswitch_both_RESOURCE_ENDPOINT, g_binaryswitch_both_nr_resource_types, 0);
  ...
  for( int a = 0; a < g_binaryswitch_both_nr_resource_types; a++ ) {
    ...
    oc_resource_bind_resource_type(res_binaryswitch_both,g_binaryswitch_both_RESOURCE_TYPE[a]);
  }
  
  oc_resource_bind_resource_interface(res_binaryswitch_both,  OC_IF_A); /* oic.if.a */
  oc_resource_bind_resource_interface(res_binaryswitch_both,  OC_IF_BASELINE); /* oic.if.baseline */
  oc_resource_bind_resource_interface(res_binaryswitch_both,  OC_IF_STARTUP); /* oic.if.startup */
  oc_resource_bind_resource_interface(res_binaryswitch_both,  OC_IF_STARTUP_REVERT); /* oic.if.startup.revert */
  ...
  oc_resource_set_request_handler(res_binaryswitch_both, OC_GET, get_binaryswitch_both, NULL);
  
  oc_resource_set_request_handler(res_binaryswitch_both, OC_POST, post_binaryswitch_both, NULL);
  
  oc_add_resource(res_binaryswitch_both);

  PRINT("Register Resource with local path \"/binaryswitch_revert\"\n");
  oc_resource_t *res_binaryswitch_revert = oc_new_resource(NULL, g_binaryswitch_revert_RESOURCE_ENDPOINT, g_binaryswitch_revert_nr_resource_types, 0);
  ...
  for( int a = 0; a < g_binaryswitch_revert_nr_resource_types; a++ ) {
    ...
    oc_resource_bind_resource_type(res_binaryswitch_revert,g_binaryswitch_revert_RESOURCE_TYPE[a]);
  }
  
  oc_resource_bind_resource_interface(res_binaryswitch_revert,  OC_IF_A); /* oic.if.a */
  oc_resource_bind_resource_interface(res_binaryswitch_revert,  OC_IF_BASELINE); /* oic.if.baseline */
  oc_resource_bind_resource_interface(res_binaryswitch_revert,  OC_IF_STARTUP_REVERT); /* oic.if.startup.revert */
  ...
  oc_resource_set_request_handler(res_binaryswitch_revert, OC_GET, get_binaryswitch_revert, NULL);
  oc_resource_set_request_handler(res_binaryswitch_revert, OC_POST, post_binaryswitch_revert, NULL);
  
  oc_add_resource(res_binaryswitch_revert);

  PRINT("Register Resource with local path \"/binaryswitch_startup\"\n");
  oc_resource_t *res_binaryswitch_startup = oc_new_resource(NULL, g_binaryswitch_startup_RESOURCE_ENDPOINT, g_binaryswitch_startup_nr_resource_types, 0);
  ...
  for( int a = 0; a < g_binaryswitch_startup_nr_resource_types; a++ ) {
    ...
    oc_resource_bind_resource_type(res_binaryswitch_startup,g_binaryswitch_startup_RESOURCE_TYPE[a]);
  }
  
  oc_resource_bind_resource_interface(res_binaryswitch_startup,  OC_IF_A); /* oic.if.a */
  oc_resource_bind_resource_interface(res_binaryswitch_startup,  OC_IF_BASELINE); /* oic.if.baseline */
  oc_resource_bind_resource_interface(res_binaryswitch_startup,  OC_IF_STARTUP); /* oic.if.startup */
  ...
  oc_resource_set_request_handler(res_binaryswitch_startup, OC_GET, get_binaryswitch_startup, NULL);
  oc_resource_set_request_handler(res_binaryswitch_startup, OC_POST, post_binaryswitch_startup, NULL);
...
initialize_variables(void)
{
  ...
  oc_storage_read("g_binaryswitch_both_storage_status", (uint8_t*)&g_binaryswitch_both_storage_status, sizeof(g_binaryswitch_both_storage_status));  
  g_binaryswitch_both_value = false; /* current value of property "value" The status of the switch. */
  ret_size = oc_storage_read("g_binaryswitch_both_value", (uint8_t*)&g_binaryswitch_both_value, sizeof(g_binaryswitch_both_value));
  ...
  oc_storage_read("g_binaryswitch_revert_storage_status", (uint8_t*)&g_binaryswitch_revert_storage_status, sizeof(g_binaryswitch_revert_storage_status));  
  g_binaryswitch_revert_value = false; /* current value of property "value" The status of the switch. */
  ret_size = oc_storage_read("g_binaryswitch_revert_value", (uint8_t*)&g_binaryswitch_revert_value, sizeof(g_binaryswitch_revert_value));
  ...
  oc_storage_read("g_binaryswitch_startup_storage_status", (uint8_t*)&g_binaryswitch_startup_storage_status, sizeof(g_binaryswitch_startup_storage_status));  
  g_binaryswitch_startup_value = false; /* current value of property "value" The status of the switch. */
  ret_size = oc_storage_read("g_binaryswitch_startup_value", (uint8_t*)&g_binaryswitch_startup_value, sizeof(g_binaryswitch_startup_value));
...
*/
  ...
  PRINT("OCF server \"server_lite_4209\" running, waiting on incoming connections.\n");
