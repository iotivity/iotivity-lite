...
POST_handler(oc_client_response_t *data)
{
  ...
  while (rep != NULL) {
	...
	PRINT("\n\n%s : %s\n\n", oc_string(rep->name), oc_string(rep->value.string));
...
get_lock_state(bool observe)
{
  ...
  if (oc_list_length(smartlocks) > 0) {
    ...
    if (c > oc_list_length(smartlocks)) {
      ...
      if (observe) {
	oc_do_observe(locks[c]->uri, locks[c]->endpoint, NULL, GET_handler, HIGH_QOS, locks[c]);
      }
      else {
	oc_do_get(locks[c]->uri, locks[c]->endpoint, NULL, GET_handler, HIGH_QOS, locks[c]);
      }
    }
  }
  else {
...
post_lock_state(void)
{
  ...
  if (oc_list_length(smartlocks) > 0) {
	...
	if (oc_init_post(locks[c]->uri, locks[c]->endpoint, NULL, &POST_handler, HIGH_QOS, locks[c])) {
