...
{   
    ...
    airpurifier_device_info = uplus_add_bridge_device(uri, OC_AIRPURIFIER_DEVICE_TYPE, name);
    g_airpurifier_device_num = oc_core_get_num_devices()-1;
    oc_str_to_uuid(AirPurifierID, &airpurifier_piid);
    oc_set_immutable_device_identifier(g_airpurifier_device_num, &airpurifier_piid);
    ...
    switch(iface_mask)
    {
        ...
        case OC_IF_A:
            ...
            if(resp == OC_STATUS_CHANGED)
                g_airpurifier.binary_switch = g_airpurifier_binary_switch;
           ...
           void *user_data)
{
    ...
    switch(iface_mask)
    {
        ...
        case OC_IF_A:
            ...
            if(resp == OC_STATUS_CHANGED)
                strcpy(g_airpurifier.mode, temp_modes);
            PRINT("mode: %s\n", g_airpurifier.mode);
            oc_new_string_array(&airpurifier_modes, (size_t)1);
            oc_string_array_add_item(airpurifier_modes, g_airpurifier.mode);
            oc_rep_set_string_array(root, modes, airpurifier_modes);
           ...
           void *user_data)
{
    ...
    switch(iface_mask)
    {
        ...
        case OC_IF_A:
            ...
            if(resp == OC_STATUS_CHANGED)
                g_airpurifier.level = g_airpurifier_targetlevel;
            break;
        default:
            resp = OC_STATUS_BAD_REQUEST;  
            break;
...
register_airpurifier_resources(void)
{
    ...
    oc_resource_t *airpurifier_binary_switch_res = oc_new_resource(NULL, "/binaryswitch", 1, g_airpurifier_device_num);
    ...
    //mode resource
    oc_resource_t *airpurifier_mode_res = oc_new_resource(NULL, "/mode", 1, g_airpurifier_device_num);
