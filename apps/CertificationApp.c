...
static oc_collection_t* createGroupResource()
{
    ...
    oc_resource_t *new_collection = oc_new_collection(collection_name, collection_uri, 1,2,2, 0);
...
void post_binaryswitchcb(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
    (void)interfaces;
    (void)user_data;
...
int main( int argc, char* argv[])
{
    ...
    if (argc > 3) {
      ...
      if (g_securityType == 3) {
        ...
        int credid = oc_pki_add_mfg_cert(0, my_crt, sizeof(my_crt), my_key,
                                         sizeof(my_key));
