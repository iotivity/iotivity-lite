...
static oc_resource_t *res[20];
static int resourceIndex = 0;
static int resourceIndexNumber;/* resourceIndexNumber: used to hold the value returned by 'createNewResource()' function*/  

static int createNewResource(const char *name, const char *uri, uint8_t num_resource_types,size_t device)
{
    res[resourceIndex] = oc_new_resource(name,uri,num_resource_types,device);
    printf("\nresourceIndex = %d\n",resourceIndex);
    resourceIndex++;
...
static void createInvisibleResource()
{
    printf("createInvisibleResource called\n");
      if (g_InvisibleResourceCreated == false) {
          ...
          resourceIndexNumber = createNewResource(NULL, FAN_INVISIBLE_URI, g_3DPrinter_nr_resource_types, 0);
          oc_resource_set_default_interface(res[resourceIndexNumber], OC_IF_RW);
          oc_resource_set_discoverable(res[resourceIndexNumber], true);
          oc_resource_set_periodic_observable(res[resourceIndexNumber], 1);
          oc_resource_set_request_handler(res[resourceIndexNumber], OC_GET, get_light, NULL);
          result = oc_add_resource(res[resourceIndexNumber]);

          if (result == true) {
              printf("Invisible Light Resource created successfully\n");
              printf("resource index = %d",resourceIndex);
...
static void createResourceWithUrl()
{
    printf("Creating Resource with complete URL\n");
    if (g_childHandle == false)
       {
            oc_resource_t *res1 = oc_ri_get_app_resource_by_uri(g_3DPrinter_RESOURCE_ENDPOINT, strlen(g_3DPrinter_RESOURCE_ENDPOINT), 0);
...
static void createSecuredResource()
{
    ...
    if (g_isSecuredResourceCreated == false) {
        ...
        resourceIndexNumber = createNewResource(NULL, g_3DPrinter_RESOURCE_ENDPOINT, g_3DPrinter_nr_resource_types, 0);
        oc_resource_set_default_interface(res[resourceIndexNumber], OC_IF_RW);
        oc_resource_set_discoverable(res[resourceIndexNumber], true);
        oc_resource_set_periodic_observable(res[resourceIndexNumber], 1);
        oc_resource_set_request_handler(res[resourceIndexNumber], OC_GET, get_light, NULL);
        res[resourceIndexNumber]->properties |= OC_SECURE;
        if (result == true) {
        ...
        resourceIndexNumber = createNewResource(NULL, g_AudioControls_RESOURCE_ENDPOINT, g_AudioControls_nr_resource_types, 0);
        oc_resource_set_default_interface(res[resourceIndexNumber], OC_IF_RW);
        oc_resource_set_discoverable(res[resourceIndexNumber], true);
        oc_resource_set_periodic_observable(res[resourceIndexNumber], 1);
        oc_resource_set_request_handler(res[resourceIndexNumber], OC_GET, get_light, NULL);
        ...
        add_device = oc_add_resource(res[resourceIndexNumber]);
        if (add_device == true) {
...
static void createManyLightResources()
{
    ...
    if (g_isManyLightCreated == false) {
        ...
        for (int i = 0; i < MAX_LIGHT_RESOURCE_COUNT; i++,lightCount++) {
            ...
            resourceIndexNumber = createNewResource(RESOURCE_NAME, uri, NUM_RESOURCES_TYPES, NDEVICE);
            oc_resource_bind_resource_type(res[resourceIndexNumber], RESOURCE_LIGHT_TYPE);
            oc_resource_bind_resource_interface(res[resourceIndexNumber], OC_IF_BASELINE);
            oc_resource_set_default_interface(res[resourceIndexNumber], OC_IF_RW);
            oc_resource_set_discoverable(res[resourceIndexNumber], true);
            oc_resource_set_periodic_observable(res[resourceIndexNumber], 1);
            oc_resource_set_request_handler(res[resourceIndexNumber], OC_GET, get_light, NULL);
...
/*
Resources created by selecting option 1 and option 3 are not being deleted at the moment
*/
static void deleteAllResources()
{
    printf("\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>'deleteAllResources' called<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<\n");

    int tempResourceIndex = resourceIndex-1;
    printf("\n Resource Index Number = %d\n", tempResourceIndex);    
    bool bool_value1 = false;

    if (g_isLightTempResourceCreated || g_isSecuredResourceCreated || g_isManyLightCreated || g_InvisibleResourceCreated) {
      
        while (tempResourceIndex >= 0)
            {
                bool_value1 = oc_delete_resource(res[tempResourceIndex]);
                if (bool_value1 == true) {
        ...
        if (tempResourceIndex < 0) {

            printf("\nAll resources deleted\n\n");
        }
    } 
    else {
        printf("\nThere is no resource available to delete!\n");
    }
}
