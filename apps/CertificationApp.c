...
static oc_resource_t *res[10];
static int resourceIndex = -1;
#define LIGHT_COUNT 2

static void createNewResource(const char *name, const char *uri, uint8_t num_resource_types,size_t device)
{
    resourceIndex++;
...
static void createInvisibleResource()
{
    ...
    if (g_InvisibleResourceCreated == false) {
        createNewResource(NULL, FAN_INVISIBLE_URI, g_3DPrinter_nr_resource_types, 0);
        oc_resource_set_default_interface(res[resourceIndex], OC_IF_RW);
        oc_resource_set_discoverable(res[resourceIndex], true);
        oc_resource_set_periodic_observable(res[resourceIndex], 1);
        oc_resource_set_request_handler(res[resourceIndex], OC_GET, get_light, NULL);
        result = oc_add_resource(res[resourceIndex]);
...
static void createSecuredResource()
{
    ...
    if (g_isSecuredResourceCreated == false) {
        ...
        createNewResource(NULL, g_3DPrinter_RESOURCE_ENDPOINT, g_3DPrinter_nr_resource_types, 0);
        oc_resource_set_default_interface(res[resourceIndex], OC_IF_RW);
        oc_resource_set_discoverable(res[resourceIndex], true);
        oc_resource_set_periodic_observable(res[resourceIndex], 1);
        oc_resource_set_request_handler(res[resourceIndex], OC_GET, get_light, NULL);
        res[resourceIndex]->properties |= OC_SECURE;
        if (result == true) {
        ...
        createNewResource(NULL, g_AudioControls_RESOURCE_ENDPOINT, g_AudioControls_nr_resource_types, 0);
        oc_resource_set_default_interface(res[resourceIndex], OC_IF_RW);
        oc_resource_set_discoverable(res[resourceIndex], true);
        oc_resource_set_periodic_observable(res[resourceIndex], 1);
        oc_resource_set_request_handler(res[resourceIndex], OC_GET, get_light, NULL);
        ...
        add_device = oc_add_resource(res[resourceIndex]);
        if (add_device == true) {
...
static void createManyLightResources()
{
    ...
    if (g_isManyLightCreated == false) {
        ...
        for (int i = 0; i < MAX_LIGHT_RESOURCE_COUNT; i++, lightCount++) {
            ...
            createNewResource(RESOURCE_NAME, uri, NUM_RESOURCES_TYPES, NDEVICE);
            oc_resource_bind_resource_type(res[resourceIndex], RESOURCE_LIGHT_TYPE);
            oc_resource_bind_resource_interface(res[resourceIndex], OC_IF_BASELINE);
            oc_resource_set_default_interface(res[resourceIndex], OC_IF_RW);
            oc_resource_set_discoverable(res[resourceIndex], true);
            oc_resource_set_periodic_observable(res[resourceIndex], 1);
            oc_resource_set_request_handler(res[resourceIndex], OC_GET, get_light, NULL);
...
static void deleteAllResources()
{
    ...
    if (g_isLightTempResourceCreated || g_isSecuredResourceCreated
        || g_isManyLightCreated || g_InvisibleResourceCreated) {
      
        while (resourceIndex >= 0){
            oc_delete_resource(res[resourceIndex]);
            resourceIndex--;
        }
        if (resourceIndex < 0) {
