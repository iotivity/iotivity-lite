...
#define set_custom_property(object, key, value) oc_rep_set_text_string(object, key, value)
...
void ReadUserdataCb(oc_rep_t* payload, char* resourceType, void** userdata)
{
    ...
    while(rep != NULL) {
        ...
        switch (rep->type) {
            case OC_REP_INT:
            {
                if(strcmp(oc_string(rep->name), stringify(USERPROPERTY_KEY_INT)) == 0) {
...
void WriteUserdataCb(oc_rep_t* payload, char* resourceType)
{
    ...
    oc_rep_set_int(root, x.user.property.int, g_userProperties.user_value_int);
    //oc_rep_set_text_string(root, USERPROPERTY_KEY_STR, g_userProperties.user_value_str);
    set_custom_property(root, USERPROPERTY_KEY_STR, g_userProperties.user_value_str);
