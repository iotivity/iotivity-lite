...
static char g_audio_RESOURCE_INTERFACE[][MAX_STRING] = {"oic.if.a","oic.if.baseline"}; /* interface if (as an array) */
int g_audio_nr_resource_interfaces = 2;

/* global resource variables for path: /scenemember1 */
static char g_scenemember_RESOURCE_INTERFACE[][MAX_STRING] = {"oic.if.baseline"}; /* interface if (as an array) */
...
static char g_ruleaction_RESOURCE_INTERFACE[][MAX_STRING] = {"oic.if.rw","oic.if.baseline"}; /* interface if (as an array) */
...
static char g_ruleexpression_RESOURCE_INTERFACE[][MAX_STRING] = {"oic.if.rw","oic.if.baseline"}; /* interface if (as an array) */
...
*/
static void
get_audio(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
  (void)user_data;  /* not used */
...
*/
static void
post_audio(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
  (void)interfaces;
  (void)user_data;
  PRINT("post_audio:\n");
...
*/
static void
get_scenemember(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
  (void)user_data;  /* not used */
...
*/
static void
get_ruleexpression(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
  (void)user_data;  /* not used */
...
*/
static void
post_ruleexpression(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
  (void)interfaces;
  (void)user_data;
...
*/
static void
get_ruleaction(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
  (void)user_data;  /* not used */
...
*/
static void
post_ruleaction(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
  (void)interfaces;
  (void)user_data;
...
register_resources(void)
{
  ...
  for( int a = 0; a < g_audio_nr_resource_interfaces; a++ ) {
    oc_resource_bind_resource_interface(res_audio, convert_if_string(g_audio_RESOURCE_INTERFACE[a]));
  }
  oc_resource_set_default_interface(res_audio, convert_if_string(g_audio_RESOURCE_INTERFACE[0]));  
  ...
  oc_resource_t *res_scenemember1 = oc_new_resource("Scene Member 1", "/scenemember1", 1, 0);
  oc_resource_bind_resource_type(res_scenemember1,"oic.wk.scenemember");
  for( int a = 0; a < g_scenemember_nr_resource_interfaces; a++ ) {
    oc_resource_bind_resource_interface(res_scenemember1, convert_if_string(g_scenemember_RESOURCE_INTERFACE[a]));
  }
  oc_resource_set_default_interface(res_scenemember1, convert_if_string(g_scenemember_RESOURCE_INTERFACE[0]));  
  oc_resource_set_discoverable(res_scenemember1, true);
  oc_resource_set_periodic_observable(res_scenemember1, 1);
  oc_resource_set_request_handler(res_scenemember1, OC_GET, get_scenemember, NULL);
  oc_add_resource(res_scenemember1);
  
  /**
  PRINT("Register Resource with local path \"/scenemember2\"\n");
  oc_resource_t* res_scenemember2 = oc_new_resource("Scene Member 2", "/scenemember2", 1, 0);
  oc_resource_bind_resource_type(res_scenemember2, "oic.wk.scenemember");
  for (int a = 0; a < g_scenemember_nr_resource_interfaces; a++) {
	  oc_resource_bind_resource_interface(res_scenemember2, convert_if_string(g_scenemember_RESOURCE_INTERFACE[a]));
  }
  oc_resource_set_default_interface(res_scenemember2, convert_if_string(g_scenemember_RESOURCE_INTERFACE[0]));
  oc_resource_set_discoverable(res_scenemember2, true);
  oc_resource_set_periodic_observable(res_scenemember2, 1);
  oc_resource_set_request_handler(res_scenemember2, OC_GET, get_scenemember, NULL);
  oc_add_resource(res_scenemember2);
  */

  PRINT("Register Resource with local path \"/scenecollection1\"\n");
  oc_resource_t *res_scenecol1 = oc_new_collection("Scene Collection 1", "/scenecollection1", 1, 0);
  ...
  oc_resource_t *res_scenelist = oc_new_collection("Scene List", "/scenelist", 1, 0);
  ...
  res_ruleexpression = oc_new_resource("Rule Expression", "/ruleexpression", 1, 0);
  ...
  for( int a = 0; a < g_ruleexpression_nr_resource_interfaces; a++ ) {
    oc_resource_bind_resource_interface(res_ruleexpression, convert_if_string(g_ruleexpression_RESOURCE_INTERFACE[a]));
  }
  oc_resource_set_default_interface(res_ruleexpression, convert_if_string(g_ruleexpression_RESOURCE_INTERFACE[0]));  
  oc_resource_set_discoverable(res_ruleexpression, true);
  oc_resource_set_periodic_observable(res_ruleexpression, 1);
  oc_resource_set_request_handler(res_ruleexpression, OC_GET, get_ruleexpression, NULL);
  oc_resource_set_request_handler(res_ruleexpression, OC_POST, post_ruleexpression, NULL);
  oc_add_resource(res_ruleexpression);
   
  PRINT("Register Resource with local path \"/ruleaction\"\n");
  oc_resource_t *res_ruleaction = oc_new_resource("Rule Action", "/ruleaction", 1, 0);
  oc_resource_bind_resource_type(res_ruleaction,"oic.r.rule.action");
  for( int a = 0; a < g_ruleaction_nr_resource_interfaces; a++ ) {
    oc_resource_bind_resource_interface(res_ruleaction, convert_if_string(g_ruleaction_RESOURCE_INTERFACE[a]));
  }
  oc_resource_set_default_interface(res_ruleaction, convert_if_string(g_ruleaction_RESOURCE_INTERFACE[0]));  
  ...
  oc_resource_set_request_handler(res_ruleaction, OC_POST, post_ruleaction, NULL);
  oc_add_resource(res_ruleaction);

  PRINT("Register Resource with local path \"/ruleinputcollection\"\n");
  oc_resource_t *res_ruleinputcol = oc_new_collection("Rule Input Collection", "/ruleinputcollection", 1, 0);
  // Remove batch from the set of supported interafaces
  res_ruleinputcol->interfaces = OC_IF_BASELINE | OC_IF_LL;
  oc_resource_bind_resource_type(res_ruleinputcol,"oic.r.rule.inputcollection");
  ...
  oc_resource_t *res_ruleactioncol = oc_new_collection("Rule Action Collection", "/ruleactioncollection", 1, 0);
  // Remove batch from the set of supported interafaces
  res_ruleactioncol->interfaces = OC_IF_BASELINE | OC_IF_LL;
  oc_resource_bind_resource_type(res_ruleactioncol,"oic.r.rule.actioncollection");
...
int init;
  ...
  PRINT("OCF server \"Rules Test Server\" running, waiting on incomming connections.\n");
