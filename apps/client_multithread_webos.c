                  ...
                  oc_resource_properties_t bm, void *user_data)
{
  ...
  for (i = 0; i < (int)oc_string_array_get_allocated_size(types); i++) {
    ...
    if (strlen(t) == 19 && strncmp(t, "oic.r.switch.binary", 10) == 0) {
...
{
  printf("find_first_endpoint\n");
  oc_endpoint_t *ep = endpoint;
  printf("[SSM] ep->version=%d\n", ep->version);
...
main(void)
{
  ...
  if (!LSRegister("org.ocf.webossample.simpleclient", &pLsHandle, &lserror)) {
...
*/
  ...
  while (quit != 1) {
    ...
    case 9:
      ...
      //oc_do_ip_discovery("/oic/res", &discovery_handler, &first_func);
      oc_do_ip_multicast("/oic/res", 0, &obt_check_owned, NULL);
      break;
    case 1:
      resource_found = false;
      printf("case 1\n");
