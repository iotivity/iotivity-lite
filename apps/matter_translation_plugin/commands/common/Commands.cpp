...
#if OC_BRG
void Commands::UnRegister(void)
{
  OC_BRG_LOG("===> unregistering commands...");
  mClusters.clear();
...
int Commands::Run(int argc, char ** argv, Command::ClientInitState init_state)
#else
...
#if OC_BRG
    if (init_state == Command::ClientInitState::COMMAND_INTERACTIVE_INIT) {
      OC_BRG_LOG("Mem & Storage Initiate !");
...
#if OC_BRG
    /*
     * <2023/11/13> Commands::Run() : insert parameter value for "interactive" of RunCommand()
     *                                             default value is "false"
     */
    switch (init_state) {
    case Command::ClientInitState::COMMAND_INTERACTIVE_INIT:
      OC_BRG_LOG("inits interactive matter client..");
      err = RunCommand(argc, argv, false, Command::ClientInitState::COMMAND_INTERACTIVE_INIT);
      break;

    case Command::ClientInitState::COMMAND_INTERACTIVE_NORMAL:
      OC_BRG_LOG("interactive matter client runs..");
      err = RunCommand(argc, argv, true);
      break;

    case Command::ClientInitState::COMMAND_INTERACTIVE_SHUTDOWN:
      OC_BRG_LOG("shutting down interactive matter client..");
      err = RunCommand(argc, argv, false, Command::ClientInitState::COMMAND_INTERACTIVE_SHUTDOWN);
      break;
...
//      printf("====>%s: initiate start!! <=====\n", __func__);
//      err = RunCommand(argc, argv, false, Command::init_state_t::COMMAND_INTERACTIVE_INIT);
//    } else if (!strcmp(argv[1], "shutdown")) {
//      printf("====>  shutdown!! <=====\n");
//      err = RunCommand(argc, argv, false, Command::init_state_t::COMMAND_INTERACTIVE_SHUTDOWN);
//    } else {
//      /* run other command interactive mode... */
//      printf("====>%s: Run interactive command !! <=====\n", __func__);
//      err = RunCommand(argc, argv, true);
//    }
#else
    /* original */
 ...
 */
CHIP_ERROR Commands::RunCommand(int argc, char ** argv, bool interactive, Command::ClientInitState init_state)
#else
...
#if OC_BRG
    /*
     * <2023/11/13> Commands::RunCommand() : modified
     */
    if (interactive)
      return command->RunAsInteractive();
    else if ((init_state == Command::ClientInitState::COMMAND_INTERACTIVE_INIT)
        || (init_state == Command::ClientInitState::COMMAND_INTERACTIVE_SHUTDOWN))
      return command->RunAsInitiate(init_state);
    else
      return command->Run();
#else
    /* original */
