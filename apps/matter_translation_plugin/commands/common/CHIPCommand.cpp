...
std::map<CHIPCommand::CommissionerIdentity,
         std::unique_ptr<chip::Controller::DeviceCommissioner>>
    CHIPCommand::mCommissioners;
...
constexpr const char *kPAATrustStorePathVariable =
    "CHIPTOOL_PAA_TRUST_STORE_PATH";
constexpr const char *kCDTrustStorePathVariable = "CHIPTOOL_CD_TRUST_STORE_PATH";

const chip::Credentials::AttestationTrustStore *CHIPCommand::sTrustStore =
    nullptr;
chip::Credentials::GroupDataProviderImpl CHIPCommand::sGroupDataProvider{kMaxGroupsPerFabric, kMaxGroupKeysPerFabric};

namespace
{
  CHIP_ERROR GetAttestationTrustStore(const char *paaTrustStorePath,
...
CHIP_ERROR CHIPCommand::MaybeSetUpStack()
{
  if (IsInteractive())
  {
    OC_BRG_LOG("interactive mode, skip setupstack!");
    return CHIP_NO_ERROR;
  }

#if OC_BRG
  if (GetInitState() == COMMAND_INTERACTIVE_SHUTDOWN)
  {
    OC_BRG_LOG("interactive mode is shutting down..,so skip setupstack!");
    return CHIP_NO_ERROR;
  }
#endif

#if OC_BRG
  if (GetInitState() == COMMAND_INTERACTIVE_INIT)
  {
  ...
  ReturnLogErrorOnFailure(
      chip::DeviceLayer::Internal::BLEMgrImpl().ConfigureBle(
          mBleAdapterId.ValueOr(0), true));
#endif
  ...
  OC_BRG_LOG("===========> enableServerInteractions: %d", factoryInitParams.enableServerInteractions);
  ...
  ReturnLogErrorOnFailure(
      DeviceControllerFactory::GetInstance().Init(factoryInitParams));

  ReturnErrorOnFailure(
      GetAttestationTrustStore(mPaaTrustStorePath.ValueOr(nullptr),
                               &sTrustStore));

  CommissionerIdentity nullIdentity{kIdentityNull, chip::kUndefinedNodeId};
  ReturnLogErrorOnFailure(
      InitializeCommissioner(nullIdentity, kIdentityNullFabricId));
  ...
  // After initializing first commissioner, add the additional CD certs once
  {
    ...
    auto additionalCdCerts = chip::Credentials::LoadAllX509DerCerts(
        cdTrustStorePath);
    if (cdTrustStorePath != nullptr && additionalCdCerts.size() == 0)
    {
      ChipLogError(chipTool,
                   "Warning: no CD signing certs found in path: %s, only defaults will be used",
                   cdTrustStorePath);
      ChipLogError(chipTool,
    ...
    ReturnErrorOnFailure(
        mCredIssuerCmds->AddAdditionalCDVerifyingCerts(additionalCdCerts));
  }
  bool allowTestCdSigningKey = !mOnlyAllowTrustedCdKeys.ValueOr(false);
  mCredIssuerCmds->SetCredentialIssuerOption(
      CredentialIssuerCommands::CredentialIssuerOptions::kAllowTestCdSigningKey,
      allowTestCdSigningKey);

  OC_BRG_LOG("setup stack finish !");
...
void CHIPCommand::MaybeTearDownStack()
{
  if (IsInteractive())
  {
    OC_BRG_LOG("interactive mode, so don't shutdown stack!");
    return;
  }

#if OC_BRG
  /*
   * <2023/11/13> CHIPCommand::MaybeTearDownStack() : added
   */
  if (GetInitState() == COMMAND_INTERACTIVE_INIT)
  {
    OC_BRG_LOG("initiating interactive matter client, so don't shutdown stack!");
    return;
  }
#endif
...
void CHIPCommand::StartTracing()
{
  ...
  {
    chip::trace::AddTraceStream(
        new chip::trace::TraceStreamFile(mTraceFile.Value()));
  }
...
void CHIPCommand::SetIdentity(const char *identity)
{
  ...
  if (name.compare(kIdentityAlpha) != 0 && name.compare(kIdentityBeta) != 0 && name.compare(kIdentityGamma) != 0 && name.compare(kIdentityNull) != 0 && strtoull(name.c_str(), nullptr, 0) < kIdentityOtherFabricId)
  {
    ChipLogError(chipTool,
                 "Unknown commissioner name: %s. Supported names are [%s, %s, %s, 4, 5...]",
...
{
  std::string name =
      mCommissionerName.HasValue() ? mCommissionerName.Value() : kIdentityAlpha;
  if (name.compare(kIdentityAlpha) != 0 && name.compare(kIdentityBeta) != 0 && name.compare(kIdentityGamma) != 0 && name.compare(kIdentityNull) != 0)
  {
...
CHIP_ERROR CHIPCommand::GetIdentityNodeId(std::string identity,
                                          chip::NodeId *nodeId)
{
...
CHIP_ERROR CHIPCommand::GetIdentityRootCertificate(std::string identity,
                                                   chip::ByteSpan &span)
{
  if (identity == kIdentityNull)
  {
...
chip::Controller::DeviceCommissioner &CHIPCommand::GetCommissioner(
    std::string identity)
{
...
CHIP_ERROR CHIPCommand::InitializeCommissioner(CommissionerIdentity &identity,
                                               chip::FabricId fabricId)
{
  std::unique_ptr<ChipDeviceCommissioner> commissioner = std::make_unique<
      ChipDeviceCommissioner>();
  chip::Controller::SetupParams commissionerParams;

  ReturnLogErrorOnFailure(
      mCredIssuerCmds->SetupDeviceAttestation(commissionerParams, sTrustStore));
  ...
  if (fabricId != chip::kUndefinedFabricId)
  {
    ...
    {
      auto option =
          CredentialIssuerCommands::CredentialIssuerOptions::kMaximizeCertificateSizes;
      mCredIssuerCmds->SetCredentialIssuerOption(option,
                                                 mUseMaxSizedCerts.Value());
    }

    ReturnLogErrorOnFailure(
        mCredIssuerCmds->InitializeCredentialsIssuer(mCommissionerStorage));
    ...
    ReturnLogErrorOnFailure(
        ephemeralKey.Initialize(chip::Crypto::ECPKeyTarget::ECDSA));

    ReturnLogErrorOnFailure(
        mCredIssuerCmds->GenerateControllerNOCChain(identity.mLocalNodeId,
                                                    fabricId, mCommissionerStorage.GetCommissionerCATs(), ephemeralKey,
  ...
  commissionerParams.operationalCredentialsDelegate =
      mCredIssuerCmds->GetCredentialIssuer();
  commissionerParams.controllerVendorId = chip::VendorId::TestVendor1;

  ReturnLogErrorOnFailure(
      DeviceControllerFactory::GetInstance().SetupCommissioner(
          commissionerParams, *(commissioner.get())));
  ...
  if (identity.mName != kIdentityNull)
  {
    ...
    ReturnLogErrorOnFailure(
        commissioner->GetCompressedFabricIdBytes(compressed_fabric_id_span));

    ReturnLogErrorOnFailure(
        chip::GroupTesting::InitData(&sGroupDataProvider, fabricIndex,
...
void CHIPCommand::RunQueuedCommand(intptr_t commandArg)
{
  ...
  CHIP_ERROR err = command->EnsureCommissionerForIdentity(
      command->GetIdentity());
...
CHIP_ERROR CHIPCommand::StartWaiting(chip::System::Clock::Timeout duration)
{
...
#if OC_BRG
  if (!IsInteractive() && (GetInitState() == COMMAND_INTERACTIVE_INIT))
  {
    OC_BRG_LOG("start Service Event..");
    ReturnLogErrorOnFailure(
        DeviceControllerFactory::GetInstance().ServiceEvents());
  }
#else
  if (!IsInteractive())
  {
  ...
  if (duration.count() == 0)
  {
    OC_BRG_LOG("run \"%s\"->RunCommand()..", GetName());
    mCommandExitStatus = RunCommand();
  }
  else
  {
    OC_BRG_LOG("schedule RunCommand()..");
    {
      std::lock_guard<std::mutex> lk(cvWaitingForResponseMutex);
      mWaitingForResponse = true;
    }

    auto err = chip::DeviceLayer::PlatformMgr().ScheduleWork(RunQueuedCommand,
    ...
    auto waitingUntil = std::chrono::system_clock::now() + std::chrono::duration_cast<std::chrono::seconds>(duration);
    {
      ...
      if (!cvWaitingForResponse.wait_until(lk, waitingUntil, [this]()
...
#if OC_BRG
  if (!IsInteractive() && (GetInitState() == COMMAND_INTERACTIVE_SHUTDOWN))
  {
    OC_BRG_LOG("stop Event Loop Task..");
