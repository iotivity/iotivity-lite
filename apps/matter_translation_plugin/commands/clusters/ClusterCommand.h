...
public:
...
#ifdef OC_BRG
        OC_BRG_LOG("============> Destination Node ID: 0x%lx, Cluster ID: 0x%x <==============", GetDestinationNodeId(), path.mClusterId);
        g_DestinationNodeId = GetDestinationNodeId();
        MatterTaskHandler::OnInvokeCommandComplete(path, data, status);
#endif
    ...
    {
#ifdef OC_BRG
        OC_BRG_LOG("============> Destination Node ID: 0x%lx, Status Code: 0x%x (%s) <==============", GetDestinationNodeId(), error.AsInteger(), error.AsString());
        g_DestinationNodeId = GetDestinationNodeId();
        MatterTaskHandler::OnInvokeCommandCompleteError(error);
#endif
    ...
    {
#ifdef OC_BRG
        OC_BRG_LOG("============> Destination Node ID: 0x%lx <==============", GetDestinationNodeId());
        g_DestinationNodeId = GetDestinationNodeId();
//        MatterTaskHandler::OnUpdateCommandComplete(path, status);
#endif
