...
void DiscoverCommissionablesCommandBase::OnDiscoveredDevice(
    const chip::Dnssd::DiscoveredNodeData &nodeData)
{
#ifdef OC_BRG
  MatterTaskHandler::OnDiscoveredDevice(nodeData);
#else
  nodeData.LogDetail();
#endif
...
CHIP_ERROR DiscoverCommissionableByShortDiscriminatorCommand::RunCommand()
{
  ...
  chip::Dnssd::DiscoveryFilter filter(
      chip::Dnssd::DiscoveryFilterType::kShortDiscriminator, mDiscriminator);
  return mCommissioner->DiscoverCommissionableNodes(filter);
}
...
CHIP_ERROR DiscoverCommissionableByLongDiscriminatorCommand::RunCommand()
{
  ...
  mCommissioner->RegisterDeviceDiscoveryDelegate(this);
  chip::Dnssd::DiscoveryFilter filter(
      chip::Dnssd::DiscoveryFilterType::kLongDiscriminator, mDiscriminator);
  return mCommissioner->DiscoverCommissionableNodes(filter);
}
...
CHIP_ERROR DiscoverCommissionableByVendorIdCommand::RunCommand()
{
  ...
  mCommissioner->RegisterDeviceDiscoveryDelegate(this);
  chip::Dnssd::DiscoveryFilter filter(
      chip::Dnssd::DiscoveryFilterType::kVendorId, mVendorId);
  return mCommissioner->DiscoverCommissionableNodes(filter);
}
...
CHIP_ERROR DiscoverCommissionableByDeviceTypeCommand::RunCommand()
{
  ...
  mCommissioner->RegisterDeviceDiscoveryDelegate(this);
  chip::Dnssd::DiscoveryFilter filter(
      chip::Dnssd::DiscoveryFilterType::kDeviceType, mDeviceType);
  return mCommissioner->DiscoverCommissionableNodes(filter);
}
