...
/*
 * modified by me <2023/11/16> enclosing_method() : added
 */
#if OC_BRG
...
{
#ifdef OC_BRG
  OC_BRG_LOG("==========> %s is successful! <==========", GetName());
#endif
...
{
#ifdef OC_BRG
  OC_BRG_LOG("==========> %s is successful! (NodeID: %lx) <==========", GetName(), nodeId);
  MatterTaskHandler::OnPairingCommandComplete(nodeId);
#endif
...
{
#ifdef OC_BRG
  OC_BRG_LOG("==========> %s is successful! (NodeID: %lx) <==========", ((PairingCommand *)context)->GetName(), nodeId);
  MatterTaskHandler::OnPairingCommandComplete(nodeId);
#endif
