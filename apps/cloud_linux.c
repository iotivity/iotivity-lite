...
#include "service/cloud-access/include/cloud_access.h"
#include "service/resource-directory/client/include/rd_client.h"
#include <pthread.h>
#include <signal.h>
#include <stdio.h>
...
{
  resource = oc_new_resource(NULL, resource_uri, 1, 0);
  oc_resource_bind_resource_type(resource, resource_rt);
  ...
  oc_resource_set_request_handler(resource, OC_GET, get_handler, NULL);
...
main(void)
{
  ...
  while (quit != 1) {
    ...
    case '1': {
      ...
      oc_sign_up(&set_ep, "github", auth_code, post_response, NULL);
      break;
    }
    case '2':
      oc_sign_in(&set_ep, oc_string(uid), oc_string(access_token),
                 post_response, NULL);
      break;
    case '3':
      oc_sign_out(&set_ep, oc_string(access_token), post_response, NULL);
      break;
    case '4':
      rd_publish(&set_ep, resource, post_response, NULL);
      break;
    case '5':
      rd_delete(&set_ep, delete_response, NULL);
      break;
    case '6':
