...
oc_sec_decode_pstat(oc_rep_t *rep, bool from_storage, int device)
{
  ...
  {
    oc_sec_otm_cb(device, OC_SEC_OTM_RESET);
  }
  else if ((ps.cm & 0x02) && false == ps.isop)
  {
    oc_sec_otm_cb(device, OC_SEC_OTM_RFOTM);
  }
  else if (false == (ps.cm & 0x02) && false == ps.isop)
  {
    oc_sec_otm_cb(device, OC_SEC_OTM_RFPRO);
  }
  else if (false == (ps.cm & 0x02) && true == ps.isop)
  {
    oc_sec_otm_cb(device, OC_SEC_OTM_RFNOP);
  }
#endif //!OC_SPEC_VER_OIC
...
post_pstat(oc_request_t *request, oc_interface_mask_t interface, void *data)
{
  ...
  if (oc_sec_decode_pstat(request->request_payload, false, device)) {
    ...
    oc_sec_otm_cb(device, OC_SEC_ERR_PSTAT);
  }
}
