...
oc_sec_encode_doxm(int device)
{
  ...
  oc_uuid_to_str(&doxm[device].devowneruuid, uuid, MAX_UUID_LENGTH);
  oc_rep_set_text_string(root, devowneruuid, uuid);
  oc_uuid_to_str(&doxm[device].deviceuuid, uuid, MAX_UUID_LENGTH);
  oc_rep_set_text_string(root, deviceuuid, uuid);
  oc_uuid_to_str(&doxm[device].rowneruuid, uuid, MAX_UUID_LENGTH);
