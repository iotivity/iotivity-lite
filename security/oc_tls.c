...
int mbedtls_ssl_read_dynamic( mbedtls_ssl_context *ssl, unsigned char **buf, size_t len )
{
    ...
    if( ret != MBEDTLS_ERR_SSL_WAITING_SERVER_HELLO_RENEGO &&
        ret != 0 )
    {
        OC_DBG("ssl_check_ctr_renegotiate returned %d", ret);
        return( ret );
    }
#endif
    ...
    if( ssl->state != MBEDTLS_SSL_HANDSHAKE_OVER )
    {
        ...
        if( ret != MBEDTLS_ERR_SSL_WAITING_SERVER_HELLO_RENEGO &&
            ret != 0 )
        {
            OC_DBG("mbedtls_ssl_handshake returned %d", ret);
            return( ret );
        }
    }
    ...
    if( ssl->in.offt == NULL )
    {
        ...
        {
            OC_DBG("set_timer to %d ms", (int) ssl->conf->read_timeout);
	    ssl->f_set_timer( ssl->p_timer, ssl->conf->read_timeout / 4, ssl->conf->read_timeout );
        }
        ...
        if( ( ret = mbedtls_ssl_read_record( ssl ) ) != 0 )
        {
            ...
            OC_DBG("mbedtls_ssl_read_record %d", ret);
            return( ret );
        }
        ...
        {
            OC_DBG("received handshake message");
            ...
            {
                OC_DBG("handshake received (not HelloRequest)");
            ...
            {
                OC_DBG("handshake received (not ClientHello)");
                                                   ...
                                                   MBEDTLS_SSL_LEGACY_NO_RENEGOTIATION ) ) )
            {
                ...
                if( ret != MBEDTLS_ERR_SSL_WAITING_SERVER_HELLO_RENEGO &&
                    ret != 0 )
                {
                    OC_DBG("ssl_start_renegotiation");
                    return( ret );
                }
            }
            else
#endif /* MBEDTLS_SSL_RENEGOTIATION */
            {
                ...
                OC_DBG("refusing renegotiation, sending alert");
    ...
    defined(MBEDTLS_SSL_PROTO_TLS1_2)
        ...
        else if( ssl->renego_status == MBEDTLS_SSL_RENEGOTIATION_PENDING )
        {
            if( ssl->conf->renego_max_records >= 0 )
            {
                ...
                {
                    OC_DBG("renegotiation requested, but not honored by client");
                    return( MBEDTLS_ERR_SSL_UNEXPECTED_MESSAGE );
                }
            }
        }
        ...
        {
            OC_DBG("ignoring non-fatal non-closure alert");
            return( MBEDTLS_ERR_SSL_WANT_READ );
        }

        if( ssl->in.msgtype != MBEDTLS_SSL_MSG_APPLICATION_DATA )
        {
            OC_DBG("bad application data message");
            return( MBEDTLS_ERR_SSL_UNEXPECTED_MESSAGE );
        }
            ...
            ssl->renego_status == MBEDTLS_SSL_RENEGOTIATION_PENDING )
        {
            ...
            {
                OC_DBG("ssl_resend_hello_request", 0);
                return( ret );
            }
        }
