           ...
           size_t identity_len)
{
  ...
  if (peer) {
    ...
    if (cred) {
      ...
      int psk_len = 0;
      oc_sec_get_rpk_psk(0, psk, &psk_len);
...
void
oc_sec_set_cpubkey_and_token_cb(oc_sec_get_cpubkey_and_token cpubkey_and_token_cb)
{
...
void
oc_sec_set_own_key_cb(oc_sec_get_own_key own_key_cb)
{
  if(NULL == own_key_cb) {
...
{
  int i = 0, ret = 0, master_key_len = 0;
  ...
  gen_master_key(master_key, &master_key_len);
