...
TEST(Security, DoxmGet)
{
  ...
  oc_request_t *request = oc_mem_malloc(sizeof(oc_request_t));
  ...
  request->query = oc_mem_malloc(request->query_len);
  snprintf(request->query, request->query_len,
           "coaps://127.0.0.1:2048/oic/sec/doxm?owned=false");
  get_doxm(request, OC_IF_BASELINE, NULL);
  snprintf(request->query, request->query_len,
           "coaps://127.0.0.1:2048/oic/sec/doxm?owned=true");
  get_doxm(request, OC_IF_BASELINE, NULL);
  snprintf(request->query, request->query_len,
           "coaps://127.0.0.1:2048/oic/sec/doxm?owner=me");
  get_doxm(request, OC_IF_BASELINE, NULL);
