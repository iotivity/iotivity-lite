...
TEST_F(TestAclOperational, Init_Platform_P)
{
  int _init_add = oc_init_platform("Samsung", NULL, NULL);
  EXPECT_EQ(0, _init_add);
}

TEST_F(TestAclOperational, Get_Con_Res_P)
{
    bool announce = true;
    oc_set_con_res_announced(announce);
    bool get_announce = oc_get_con_res_announced();
    EXPECT_EQ(true, get_announce);
  }