...
TEST(Security, AclPost)
{
  ...
  request->response->response_buffer->buffer =
    (uint8_t *)malloc(request->response->response_buffer->response_length);
...
{
  oc_tls_peer_t *peer = (oc_tls_peer_t *)malloc(sizeof(oc_tls_peer_t));
  EXPECT_TRUE(peer);
  mbedtls_ssl_init(&peer->ssl_ctx);
  OC_LIST_STRUCT_INIT(peer, send_q);
  oc_message_t *message = (oc_message_t *)malloc(sizeof(oc_message_t));
  EXPECT_TRUE(message);
  oc_list_push(peer->send_q, message);
  OC_LIST_STRUCT_INIT(peer, recv_q);
  oc_message_t *message1 = (oc_message_t *)malloc(sizeof(oc_message_t));
...
TEST(Security, TlsSetTimers)
{
  oc_tls_retr_timer_t *timer =
    (oc_tls_retr_timer_t *)malloc(sizeof(oc_tls_retr_timer_t));
