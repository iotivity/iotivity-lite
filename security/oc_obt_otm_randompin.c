...
obt_rdp_12(oc_client_response_t *data)
{
  ...
  const oc_device_t *device = o->device;
  const oc_endpoint_t *ep = oc_obt_get_secure_endpoint(device->endpoint);
  if (oc_init_post(OCF_SEC_ACL_URI, ep, NULL, &obt_rdp_13, HIGH_QOS, o)) {
...
obt_rdp_11(oc_client_response_t *data)
{
  ...
  const oc_device_t *device = o->device;
  const oc_endpoint_t *ep = oc_obt_get_secure_endpoint(device->endpoint);
  if (oc_do_delete(OCF_SEC_ACL_URI, ep, NULL, &obt_rdp_12, HIGH_QOS, o)) {
    return;
  }
...
obt_rdp_7(oc_client_response_t *data)
{
  ...
  bool derived = oc_sec_derive_owner_psk(
    ep, (const uint8_t *)OC_OXMTYPE_RDP_STR,
    OC_CHAR_ARRAY_LEN(OC_OXMTYPE_RDP_STR), device->uuid.id,
   ...
   */
  if (oc_init_post(OCF_SEC_CRED_URI, ep, NULL, &obt_rdp_8, HIGH_QOS, o)) {
    oc_rep_start_root_object();
...
obt_rdp_5(oc_client_response_t *data)
{
  ...
  if (oc_init_post(OCF_SEC_ACL_URI, ep, NULL, &obt_rdp_6, HIGH_QOS, o)) {
    const oc_uuid_t *my_uuid = oc_core_get_device_id(0);
    char uuid[OC_UUID_LEN];
    oc_uuid_to_str(my_uuid, uuid, OC_UUID_LEN);
...
obt_rdp_3(oc_client_response_t *data)
{
   ...
   */
  char suuid[37];
  oc_uuid_to_str(&device->uuid, suuid, OC_ARRAY_SIZE(suuid));
...
*/
int
oc_obt_perform_random_pin_otm(const oc_uuid_t *uuid, const unsigned char *pin,
                              ...
                              void *data)
{
  ...
  char subjectuuid[37];
  oc_uuid_to_str(uuid, subjectuuid, OC_ARRAY_SIZE(subjectuuid));
  ...
  oc_sec_encoded_data_t privatedata = { key, OC_ARRAY_SIZE(key),
                                        OC_ENCODING_RAW };
  int credid =
    oc_sec_add_new_psk_cred(0, subjectuuid, privatedata, OC_STRING_VIEW_NULL);
  if (credid == -1) {
    oc_obt_free_otm_ctx(o, -1, OC_OBT_OTM_RDP);
    return -1;
  }
...
*/
int
oc_obt_request_random_pin(const oc_uuid_t *uuid, oc_obt_device_status_cb_t cb,
                          void *data)
{
