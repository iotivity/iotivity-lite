...
function doMemoryProfile()
{
	...
	do
		...
		massiffile=$(echo "$var" | tr '/' ',').massif.log.out.txt

		ms_print massif.out.* >> "${iotivity}/memory_profile/$massiffile"
		memoryfile=$(echo "$var" | tr '/' ',').memory.log.out.txt
...
function doMemoryTrace()
{
	...
	for var in "${POSITIONAL[@]}"
	do
		unrechablememoryfile=$(echo "$var" | tr '/' ',').unrechablememoryfile.log.out.txt
		./$var >> "${iotivity}/memory_profile/$unrechablememoryfile"
	done

	doUpdateConfirFile 2
}
