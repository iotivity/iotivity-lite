...
int RIHelper::initServer() {
	...
	int ret = 0;
	s_handler = {.init = appInitCb,
		.signal_event_loop = signalEventLoopCb,
		.register_resources = registerEmptyResourcesCb
	};
...
static void RIHelper::getLightCb(oc_request_t *request,
...
static void RIHelper::postLightCb(oc_request_t *request,
		oc_interface_mask_t interface, void *user_data) {
	PRINT("postLightCb:\n");
	bool state = false;
...
static void RIHelper::putLightCb(oc_request_t *request,
		oc_interface_mask_t interface, void *user_data) {
...
static oc_event_callback_retval_t RIHelper::stopObserveClientCb(void *data) {
	(void) data;
...
static void RIHelper::observeLightCb(oc_client_response_t *data) {
...
static void RIHelper::getLightClientCb(oc_client_response_t *data) {
	PRINT("getLightClientCb\n");
	s_isRequestSucessfull = true;
	s_generalQuit=1;
}

static void RIHelper::postLightClientCb(oc_client_response_t *data) {
...
static void RIHelper::putLightClientCb(oc_client_response_t *data) {
...
static void RIHelper::deleteLightClientCb(oc_client_response_t *data) {
...
static oc_discovery_flags_t RIHelper::discovery(const char *di, const char *uri,
...
void RIHelper::getResource() {
	...
	oc_do_get(s_lightUri, s_pLightEndpoint, NULL, &getLightClientCb, LOW_QOS, NULL);
}
...
void RIHelper::deleteResource() {
	...
	s_generalQuit = 0;
	s_isRequestSucessfull = false;
	oc_do_delete(s_lightUri, s_pLightEndpoint, &deleteLightClientCb, LOW_QOS, NULL);
}
void RIHelper::postRequestResource() {
	PRINT("postRequestResource:\n");
	s_generalQuit = 0;
	s_isRequestSucessfull = false;
	if (oc_init_post(s_lightUri, s_pLightEndpoint, NULL, &postLightClientCb, LOW_QOS,
	NULL)) {
		oc_rep_start_root_object();
		oc_rep_set_boolean(root, state, !s_lightState);
		oc_rep_end_root_object();
...
void RIHelper::putRequestResource() {
	...
	s_generalQuit = 0;
	s_isRequestSucessfull = false;
	if (oc_init_put(s_lightUri, s_pLightEndpoint, NULL, &putLightClientCb, LOW_QOS,
	NULL)) {
		oc_rep_start_root_object();
		oc_rep_set_boolean(root, state, !s_lightState);
		oc_rep_end_root_object();
...
void RIHelper::observeResource() {
	...
	s_generalQuit = 0;
	oc_do_observe(s_lightUri, s_pLightEndpoint, NULL, &observeLightCb, LOW_QOS, NULL);
