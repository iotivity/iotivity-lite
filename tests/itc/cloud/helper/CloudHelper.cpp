...
void CloudHelper::getLightCb(oc_request_t *request, oc_interface_mask_t interface,
		void *user_data) {
	PRINT("getLightCb:\n");
	(void) user_data;
...
static void
CloudHelper::cloudPostResponseCb(oc_client_response_t *data)
{
  if (data->code == OC_STATUS_CHANGED)
...
static void
CloudHelper::parsePayload(oc_client_response_t *data)
{
  oc_rep_t *rep = data->payload;
  while (rep != NULL) {
...
bool CloudHelper::ocSignUpWithAuth(const char *provier, const char *address,const char *auth_code)
{
	s_generalQuit = 0;
    ...
    return oc_sign_up_with_auth(&s_cloudEndpoint, provier, auth_code, 0, cloudPostResponseCb,
                         NULL);
}
...
bool CloudHelper::ocSignInWithAuth()
{
	...
	return oc_sign_in(&s_cloudEndpoint, oc_string(uid), oc_string(access_token), 0,   cloudPostResponseCb, NULL);
}

bool CloudHelper::ocSignInWithAuth(bool endpoint, const char *luid, const char *laccess_token, bool cb)
{
	s_generalQuit = 0;
	s_isRequestSucessfull = 0;

	if(endpoint)
		return oc_sign_in(NULL, oc_string(uid),  oc_string(access_token), 0, cloudPostResponseCb, NULL);
	else if(cb)
		return oc_sign_in(&s_cloudEndpoint, oc_string(uid),  oc_string(access_token), 0, NULL, NULL);
	else
		return oc_sign_in(&s_cloudEndpoint, luid, laccess_token, 0, cloudPostResponseCb, NULL);
}
...
bool CloudHelper::ocSignOutWithAuth()
{
	s_generalQuit = 0;
	s_isRequestSucessfull = 0;
	return oc_sign_out(&s_cloudEndpoint, oc_string(access_token), 0, cloudPostResponseCb, NULL);
}

bool CloudHelper::ocSignOutWithAuth(bool endpoint, const char *laccess_token, bool cb)
{
	s_generalQuit = 0;
	s_isRequestSucessfull = 0;
	if(endpoint)
		return oc_sign_out(NULL, oc_string(access_token), 0, cloudPostResponseCb, NULL);
	else if(cb)
		return oc_sign_out(&s_cloudEndpoint, oc_string(access_token), 0, NULL, NULL);
	else
		return oc_sign_out(&s_cloudEndpoint, laccess_token, 0, cloudPostResponseCb, NULL);
...
bool CloudHelper::ocSignUp()
{
	s_generalQuit = 0;
	s_isRequestSucessfull = 0;
	return oc_sign_up(&s_cloudEndpoint,"github", oc_string(uid), oc_string(access_token), 0, cloudPostResponseCb, NULL);
}

bool CloudHelper::ocSignUp(bool endpoint,const char *lauthprovider, const char *luid, const char *laccess_token, bool cb)
{
	s_generalQuit = 0;
	s_isRequestSucessfull = 0;

	if(endpoint)
		return oc_sign_up(NULL,"github", oc_string(uid), oc_string(access_token), 0, cloudPostResponseCb, NULL);
	else if(cb)
		return oc_sign_up(&s_cloudEndpoint,"github", oc_string(uid), oc_string(access_token), 0, NULL, NULL);
	else
		return oc_sign_up(&s_cloudEndpoint,lauthprovider, luid, laccess_token, 0, cloudPostResponseCb, NULL);
}
