...
int sockfd = 0, n = 0;
    ...
    if( connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0){
       printf("\n TAFAgent Error : Connect Failed \n");
       return;
    }
        printf("TAF: Enter Data for CertApp Server: %s\n", cmd);
...
void FindKey(char *cmd, char *key){
    ...
    if( connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0){
       printf("\n TAF Error : Connect Failed \n");
       return;
    }
        printf("TAFAgent: Enter Data for : %s\n", cmd);
            if((send(sockfd,cmd, strlen(cmd),0))== -1){
                fprintf(stderr, "Failure Sending Message\n");
                close(sockfd);
                return;
        ...
        if((client_fd = accept(listenfd, (struct sockaddr *)&dest, &size))==-1 ){
            ...
            while(1){
                ...
                if((num = recv(client_fd, buffer, 1024,0))== -1){
                        ...
                        printf("TAF Agent Sever: Connection closed\n");
                        //So I can now wait for another client
                        break;
                }
                printf("TAFServer: num:%d",num);
                buffer[num] = '\0';
                printf("TAFServer: buffer:%s\n", buffer);
...
int main(int argc, char **argv){
    ...
    int ipVerArg = atoi(argv[3]);
    if(argc < 12){
    printf("ERROR: Wrong Command or Amount of Arguments, Shoule Be:\n\nExample:\n[./TAFAgent conf_path QoS IP_Version Security OCF_Version IP_Address InterfaceIndex ScopeID CertAppInput1 CertAppInput2 TAF_Mode]\n");
    ...
    Scope_ID -Of the Machine Running CTT\nCertAppInput1:\t1 -Create a Resource\nCertAppInput2:\t1 -Create a Resource\nTAF_Mode:\tserver -To Start TAF in Server Mode\n\
   ...
   printf("Failed to create main thread\n");
    ...
    while(!gQuitFlag){
      if(!gIPDiscovered){
        ...
        if (strcmp(argv[11], "client") == 0){
            SendCommand("12");
            sleep(SLEEP_TIME);
            printf("\nEntering TAF Agent main loop Client Mode...\n");
        }
        if (strcmp(argv[11], "server") == 0){
            printf("\nEntering TAF Agent main loop Server Mode...\n");
        }
