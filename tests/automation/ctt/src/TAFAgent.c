...
//while(1) {
        printf("Client: Enter Data for Server: %s\n", cmd);
...
void initIUT(int qosArg, int ipVerArg, int securityArg, char *ocfVerArg, char initActions[][5], int initActionsSize)
{
    ...
    //char *args[]={"./CertificationApp","1 2 3"}; 
    if ((gPid = fork()) < 0)
    {
        printf("Fork failed \n");
        exit(1);
...
do{
	...
	FindValue("Endpoint CTT Discovery:",path);
	printf("Path:%s",path);
	if(strcmp(path,"") !=0)
	{
		 sleep(2);
...
int ipVerArg = atoi(argv[3]);
    if (argc < 6)
    {
        printf("Error: Wrong amount of arguments, shoule be:\n./TAFAgent conf_path QoS ipVersion security ocfVersion IPv4AddressWhereTAFAgentRuns\n");
        return -1;
    }
