...
char gIUTlog[100] = "CertAppLog.txt";
bool gSecured = false;
char gIp[30];
char gLocalIp[50];
char gPort[25];
...
/*Replace String*/
char *
replaceString(const char *str, const char *from, const char *to)
{
  ...
  if (count == 0) {
    ...
    for (i = 0; i < count; i++) {
      ...
      cpyLen =
        (i == count - 1 ? orgLen : posCache[i + 1]) - posCache[i] - fromLen;
...
void
FindValue(char *searchString, char *value)
{
  ...
  if (fp1) {
    while ((read = getline(&line, &len, fp1)) != -1) {
      ++lineNo;
      if ((strstr(line, searchString) != NULL) && (strstr(line, ":") != NULL)) {
...
void
FindKey(char *searchString, char *key)
{
  FILE *fp1 = fopen(gIUTlog, "r");
  char *line = NULL;
  int i, j;
...
void
SendCommands(int commandLength)
{
...
ActionHandler(xmlDocPtr doc)
{
  ...
  if (strstr(messageXml, "<message>Waiting for CoAP response... ") != NULL) {
    // SKIP
                    ...
                    "<message>Please send a CoAP Ping message to the CTT") !=
             NULL) {

    char tempString[256];
    char delimiters[] = "()";
    char cmdString[43] = "coap+tcp://";
    ...
    SendCommand(cmdString);
                    ...
                    "<message>If IUT uses an OCF Rooted Certificate Chain") !=
             NULL) {
    asprintf(&result, "<actionResult><userDialogResponse><answer>Skip</"
                      "answer></userDialogResponse></actionResult>");
  } else if (strstr(
               messageXml,
               "<message>Please provide (paste) public key of CA (PEM type)") !=
             NULL) {
    ...
    FindValue("Random PIN: ", gPinValue);
    printf("\n%s\n", gPinValue);
    ...
    strstr(
      messageXml,
      "<message>Please initiate device to revert to \"ready for OTM\" state") !=
      NULL ||
    strstr(messageXml,
           "<message>Please reset DUT's ACL in order to have empty list.") !=
      NULL) {
    ...
    asprintf(&result, "<actionResult><userDialogResponse><answer>OK</answer></"
                      "userDialogResponse></actionResult>");
  } else if (strstr(messageXml, "<message>Please cycle power on the IUT") !=
             NULL) {
    /* Reset the CertificationApp */
    asprintf(&result, "<actionResult><userDialogResponse><answer>OK</"
                      "answer></userDialogResponse></actionResult>");
  } else if (strstr(messageXml,
                    "Please revert IUT to RFOTM / reset the device") != NULL) {
    /* Reset the CertificationApp */
    gRestartFlag = 1;
    gReuseIUT = 1;
    gIPDiscovered = 0;
    ...
    asprintf(&result, "<actionResult><userDialogResponse><answer>OK</answer></"
                      "userDialogResponse></actionResult>");
  } else if (strstr(messageXml, "<message>Please change some value in "
                                "/BinarySwitchResURI resource") != NULL) {
    ...
    if (strstr(messageXml, "and press OK") != NULL) {
      ...
      SendCommands(5);
                    ...
                    "<message>If the device uses an OCF Rooted Certificate "
                    "Chain, click 'Skip'. Otherwise please provide (paste) the "
                    "CA (PEM type) known to the device") != NULL) {
    asprintf(&result, "<actionResult><userDialogResponse><answer>Skip</"
                      "answer></userDialogResponse></actionResult>");
  } else if (strstr(messageXml, "<message>Please change some value in "
                                "/binaryswitch resource and press OK") !=
             NULL) {
    ...
    asprintf(&result, "<actionResult><userDialogResponse><answer>OK</answer></"
                      "userDialogResponse></actionResult>");
                                ...
                                "/humidity resource and press OK") != NULL) {
    sleep(2);
    asprintf(&result, "<actionResult><userDialogResponse><answer>OK</answer></"
                      "userDialogResponse></actionResult>");
    ...
    asprintf(&result, "<actionResult><userDialogResponse><answer>Yes</answer></"
                      "userDialogResponse></actionResult>");
  } else if (strstr(messageXml,
                    "<message>Was device discovered sucessfully?") != NULL) {
    ...
    if (strstr(messageXml, "and press OK") != NULL) {
      ...
      SendCommands(5);
             ...
             messageXml,
             "<message>Please change some value in /HumidityResURI resource") !=
           NULL) {
    if (strstr(messageXml, "and press OK") != NULL) {
  ...
  } else if (strstr(messageXml, "<message>Please send a multicast discovery "
                                "request message (i.e. CoAP GET) to") != NULL ||
             strstr(messageXml,
    ...
    asprintf(&result, "<actionResult><userDialogResponse><answer>Skip</"
               ...
               messageXml,
               "<message>Please have the IUT establish a TCP connection") !=
             NULL) {
    SendCommand("12");
    sleep(2 * SLEEP_TIME);
    SendCommand("17");
    ...
    asprintf(&result, "<actionResult><userDialogResponse><answer>OK</answer></"
                    ...
                    " send a unicast RETRIEVE request (i.e. CoAP GET) to ") !=
             NULL) {
    if (strstr(messageXml, "/BinarySwitchResURI") != NULL) {
      ...
      asprintf(&result, "<actionResult><userDialogResponse><answer>CANCEL</"
                        "answer></userDialogResponse></actionResult>");
    }
    ...
    sleep(2 * SLEEP_TIME);
                    ...
                    "Please provide the Mediator with the generated cis") !=
             NULL) {
    asprintf(&result, "<actionResult><userDialogResponse><answer>OK</answer></"
                      "userDialogResponse></actionResult>");
  } else if (strstr(messageXml,
                    "Please initiate transfer of Cloud configuration") !=
             NULL) {
    SendCommand("12");
    sleep(SLEEP_TIME);
    SendCommand("39");
    sleep(SLEEP_TIME);
  } else if (strstr(messageXml, "<message>Does IUT have the possibility to "
                                "display received properties values?") !=
             NULL) {
    asprintf(&result, "<actionResult><userDialogResponse><answer>Yes</answer></"
                      "userDialogResponse></actionResult>");
  } else if (strstr(messageXml,
                    "<message>Does IUT has discovered a device (i.e. the CTT) "
                    "with the expected Resource Types?") != NULL) {
    asprintf(&result, "<actionResult><userDialogResponse><answer>Yes</answer></"
                      "userDialogResponse></actionResult>");
  } else if (strstr(
               messageXml,
               "<message>Does IUT presents following properties values?") !=
             NULL) {
    asprintf(&result, "<actionResult><userDialogResponse><answer>Yes</answer></"
                      "userDialogResponse></actionResult>");
  } else {
...
xmlDocPtr
AbortSignalHandler(xmlDocPtr doc)
{
  printf("TAF==Received GET /abortSignal: \n");
  return stringToDoc(
    "<abortSignal><abortTestcase>false</abortTestcase><abortTestRun>false</"
    "abortTestRun></abortSignal>");
}

/*Configuration handler*/
xmlDocPtr
ConfigurationHandler(xmlDocPtr doc)
{
...
InitTAF()
{
  ...
  if (gConfigPath != NULL) {
    addRouteSetup("GET", "/processes/ctt/configuration", ConfigurationHandler,
                  CleanXmlDocPtr);
  }
...
void
initIUT(int qosArg, int ipVerArg, int securityArg, char *ocfVerArg,
        char initActions[][5], int initActionsSize)
{
...
void
startIUTfromArguments(int argc, char **argv)
{
  ...
  if (gReuseIUT) {
    ...
    initIUT(qosArg, ipVerArg, securityArg, ocfVerArg, initialCommands,
            initialCommandsSize);
  }
}
...
main(int argc, char **argv)
{
  ...
  if (argc < 6) {
    printf("Error: Wrong amount of arguments, shoule be:\n./TAFAgent conf_path "
           "QoS ipVersion security ocfVersion\n");
    return -1;
  }
