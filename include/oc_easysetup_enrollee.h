 ...
 */
typedef enum {
  OC_WES_RESOURCE = 0x01,
  OC_WES_WIFICONF_RESOURCE = 0x02,
  OC_WES_DEVCONF_RESOURCE = 0x04
} oc_wes_resource_mask_t;
...
typedef enum {
   ...
   * Init Error Code.
   */
  OC_EES_ERRCODE_NO_ERROR = 0,
  /**
   * Fialed to read eUICC Info by LPA
   */
  OC_EES_ERRCODE_EUICC_INFO_READ_FAILED,
  /**
   * Fialed to read Device Info by LPA
   */
  OC_EES_ERRCODE_DEVICE_INFO_READ_FAILED,
  /**
   * Failed to install eUICC profile
   */
  OC_EES_ERRCODE_INSTALL_PROFILE_FAILED,
  /**
   * Invalid Activation Code
   */
  OC_EES_ERRCODE_ACTIVATION_CODE_INVALID,
  /**
   * Unknown error occured.
   */
  OC_EES_ERRCODE_UNKNOWN = 255
} oc_ees_error_code;
 ...
 */
typedef enum {
  OC_EES_RESOURCE = 0x01,
  OC_EES_RSPCONF_RESOURCE = 0x02,
  OC_EES_RSPCAPCONF_RESOURCE = 0x04
} oc_ees_resource_mask_t;
...
/**
 * A callback function to clean up user data created in oc_wes_wifi_data,
 * oc_wes_device_data and es_coap_cloud_conf_data.
 *
...
// WiFi Easy setup provisioning callbacks
typedef void (*oc_wes_prov_cb)(oc_wes_data *);
typedef void (*oc_wes_wifi_prov_cb)(oc_wes_wifi_data *);
typedef void (*oc_wes_dev_prov_cb)(oc_wes_device_data *);
 ...
 * oc_wes_wifi_data, oc_wes_device_data.
 *
 * @return ::OC_WES_OK on success, some other value upon failure.
 *
 ...
 * oc_wes_data, oc_wes_wifi_data, oc_wes_device_data.
 *
 * @return ::OC_WES_OK on success, some other value upon failure.
 *
 * @see oc_es_read_userdata_cb_t
 ...
 * @return ::OC_WES_OK on success, some other value upon failure.
 *
 * @see oc_wes_device_info
 */
oc_es_result_t oc_wes_set_device_info(size_t device, oc_wes_device_info *device_info);
 ...
 * @return ::OC_WES_OK on success, some other value upon failure.
 *
 * @see oc_wes_error_code
 */
oc_es_result_t oc_wes_set_error_code(size_t device, oc_wes_error_code err_code);
 ...
 */
oc_es_result_t oc_wes_set_state(size_t device, oc_es_enrollee_state es_state);
 ...
 * @see oc_es_enrollee_state
 */
oc_es_enrollee_state oc_wes_get_state(size_t device);

// eSIM Easy setup provisioning callbacks
typedef void (*oc_ees_prov_cb)(oc_ees_data *);
typedef void (*oc_ees_rsp_prov_cb)(oc_ees_rsp_data *);
typedef void (*oc_ees_rspcap_prov_cb)(oc_ees_rspcap_data *);
 ...
 * @param writecb a callback for putting properties to a response to be sent
 * @param freecb callback to free allocated memory of user data in
 * oc_ees_data, oc_ees_rsp_data and oc_ees_rspcap_data.
 *
 * @return ::OC_EES_OK on success, some other value upon failure.
 *
 * @see oc_es_read_userdata_cb_t
 ...
 * @see oc_wes_device_info
 */
oc_es_result_t oc_ees_set_device_info(size_t device, oc_ees_device_info *device_info);
 ...
 * @return ::OC_EES_OK on success, some other value upon failure.
 *
 * @see oc_ees_error_code
 */
oc_es_result_t oc_ees_set_error_code(size_t device, oc_ees_error_code err_code);
 ...
 * @see oc_es_enrollee_state
 */
oc_es_result_t oc_ees_set_state(size_t device, oc_es_enrollee_state es_state);
 ...
 * @see oc_es_enrollee_state
 */
oc_es_enrollee_state oc_ees_get_state(size_t device);
