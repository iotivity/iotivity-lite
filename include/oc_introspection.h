 ...
 * be added to the oc_storage before the server starts using the name IDD_<device_index>.
 * Or it can be added at run time by passing in the IDD data as a byte array calling
 * oc_set_introspection_data. The IDD information is served up as encoded CBOR contents
 * (e.g. read as is). If the size of the IDD data is to big for the buffer, then an internal
 * error is given back.
 * note that this option can serve up more than 1 introspection file, if multiple devices are implemented.
 * This feature is enabled when the compile switch OC_IDD_API is used.
 *
