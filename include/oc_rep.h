...
/**
 * Print the contents of oc_rep_t
 *
 ...
 * @param[in]  pretty_print if true extra white space and new lines will be added to
 *                          the output making it more human readable. Note return
 *                          value will differ if pretty_print value is changed.
 *
 * Example:
 * ~~~{.c}
 *     char * json;
 ...
 *
 * The function will not write more than buf_size bytes (including the terminating null byte ('\0')).
 * If the output was truncated due to this limit then the return value is the number of characters
 * (excluding the terminating null byte) which would have been written to the final string if
 * enough space had been available. Thus, a return value of buf_size or more means that the output
 * was truncated.
 */
