 ...
 * ~~~
 *
 * See oc_rep_add_int to see the recommended way to do the same thing using
 * `oc_rep_open_array` / `oc_rep_close_array` instead.
 *
 * @see oc_rep_set_key
 * @see oc_rep_end_array
...
#define oc_rep_open_object(object, key)                                        \
  g_err |= cbor_encode_text_string(&object##_map, #key, strlen(#key));         \
  oc_rep_begin_object(object##_map, key)
