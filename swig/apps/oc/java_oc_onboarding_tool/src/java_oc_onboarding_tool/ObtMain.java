...
import java_onboarding_tool.DeleteAceByAceIdHandler;
import java_onboarding_tool.DeleteCredentialIdHandler;
import java_onboarding_tool.OCFDeviceInfo;
import java_onboarding_tool.ResourceDiscovery;
import java_onboarding_tool.RetrieveAcl2Handler;
import java_onboarding_tool.RetrieveCredintialResourceHandler;
...
public class ObtMain {
    ...
    public static void discoverResources()
    {
        ...
        int ret = OCObt.discoverAllResources(devices[userInput].getUuid(), new ResourceDiscovery());
        if (ret >= 0)
        {
    ...
    public static void displayCredentialResource(OCCreds creds)
    {
        if (creds != null) {
            ...
            while (cr != null) {
              ...
              if (!cr.getRole().getRole().isEmpty()) {
                  System.out.println("roleid_role: " + cr.getRole().getRole());
              }
              if (!cr.getRole().getAuthority().isEmpty()) {
                System.out.println("roleid_authority: " + cr.getRole().getAuthority());
              }
              System.out.println("\n-----");
              cr = cr.getNext();
            }
    ...
    public static void retrieveCredentialResource()
    {
        ...
        int ret = OCObt.retrieveCreds(ods[userInput].getUuid(), new RetrieveCredintialResourceHandler());
        if (ret >= 0) {
    ...
    public static void deleteCredetialByCredentialId()
    {
        ...
        int ret = OCObt.deleteCredByCredId(ods[userInput].getUuid(), credid, new DeleteCredentialIdHandler());
        if (ret >= 0) {
    ...
    public static void retrieveAce2Resource()
    {
        ...
        int ret = OCObt.retrieveAcl(ods[userInput].getUuid(), new RetrieveAcl2Handler());
        if (ret >= 0) {
    ...
    public static void deleteAceByAceId()
    {
        ...
        int ret = OCObt.deleteAceByAceId(ods[userInput].getUuid(), aceid, new DeleteAceByAceIdHandler());
        if (ret >= 0) {
