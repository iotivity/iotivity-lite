...
oboard and provision samples even if they are running on a remote device. The only requirement is
...
### (Step 1) Onboard and Provision the Server
 ...
 - Type `5` **Enter** to _Provision ACE2_. There are many ways to properly provision the device.
   ...
   - Type `1` **Enter** in responce to `Enter number of resources in this ACE:`
   - Type `0` **Enter** in responce to `Have resource href? [0-No, 1-Yes]:`
   - Type `1` **Enter** in responce to `Set wildcard resource? [0-No, 1-Yes]:`
   - Type `2` **Enter** to select the `[2]: All discoverable resources` option
   - Type `0` **Enter** in responce to `Enter number of resource types [0-None]:`
   - Type `0` **Enter** in responce to `Enter number of interfaces [0-None]`
