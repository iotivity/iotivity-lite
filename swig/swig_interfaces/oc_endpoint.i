...
%javaexception("OCEndpointParceException") jni_string_to_endpoint {
  $action
  /* The `oc_endpoint_t *endpoint` parameter is jarg1, the name is generated by SWIG. */
  if(result == NULL) {
    OC_DBG("JNI: String can not be parced.");
    jclass cls_OCEndpointParceException = JCALL1(FindClass, jenv, "org/iotivity/OCEndpointParceException");
    assert(cls_OCEndpointParceException);
    jmethodID mid_OCEndpointParceException_init = JCALL3(GetMethodID, 
                                                         jenv,
                                                         cls_OCEndpointParceException,
                                                         "<init>",
                                                         "(Ljava/lang/String;Ljava/lang/String;)V");
    assert(mid_OCEndpointParceException_init);
    jstring jmessage = JCALL1(NewStringUTF, jenv, "String cannot be parced.");
    /* jarg1 is the name given by SWIG to the passed in string from Java */
    jobject jEndpointParceException = JCALL4(NewObject,
                                             jenv,
                                             cls_OCEndpointParceException,
                                             mid_OCEndpointParceException_init,
                                             jmessage,
                                             jarg1);
    assert(jEndpointParceException);
    JCALL1(Throw, jenv, jEndpointParceException);
    //(*jenv)->ThrowNew(jenv, clazz, "OCEndpoint cannot be null.", jarg1);
    return $null;
  }
}
