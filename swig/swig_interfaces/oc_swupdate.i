...
static jni_callback_data jni_swupdate_handler;
%}
...
int jni_validate_purl(const char *url)
{
  ...
  const jmethodID mid_validate_purl = JCALL3(GetMethodID,
                                       (jni_swupdate_handler.jenv),
                                       cls_OCSoftwareUpdateHandler,
                                       "validatePURL",
                                       "(Ljava/lang/String;)I");
  assert(mid_validate_purl);
  jstring jurl = JCALL1(NewStringUTF, (data->jenv), (const char *)url);
  jint return_value = JCALL3(CallIntMethod, (jni_swupdate_handler.jenv),
                             jni_swupdate_handler.jcb_obj,
                             mid_validate_purl,
                             jurl);
...
int jni_check_new_version(size_t device, const char *url, const char *version)
{
  ...
  jstring jurl = JCALL1(NewStringUTF, (data->jenv), url);
  jstring jversion = JCALL1(NewStringUTF, (data->jenv), version);
  jint return_value = JCALL5(CallIntMethod, (jni_swupdate_handler.jenv),
                             jni_swupdate_handler.jcb_obj,
...
int jni_download_update(size_t device, const char *url)
{
  ...
  jstring jurl = JCALL1(NewStringUTF, (data->jenv), url);
  jint return_value = JCALL4(CallIntMethod, (jni_swupdate_handler.jenv),
                             jni_swupdate_handler.jcb_obj,
                             mid_download_update,
                             (jlong) device,
                             jurl);
...
%typemap(in)     const oc_swupdate_cb_t *swupdateImpl {
  ...
  jni_swupdate_handler.jenv = jenv;
  jni_swupdate_handler.jcb_obj = JCALL1(NewGlobalRef, jenv, $input);
