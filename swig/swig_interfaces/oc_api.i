...
%typemap(jni) (int **value, size_t *size) "jobjectArray"
%typemap(jtype) (int **value, size_t *size) "String[]"
%typemap(jstype) (int **value, size_t *size) "String[]"
%typemap(javain) (int **value, size_t *size) "$javainput"
%typemap(javadirectorin) (int **value, size_t *size) "$javainput"
%typemap(javadirectorout) (int **value, size_t *size) "$javacall"

%typemap(in) (int **value, size_t *size) ($*1_ltype temp_value, $*2_ltype temp_size) {
  if (!$input) {
    SWIG_JavaThrowException(jenv, SWIG_JavaNullPointerException, "array null");
    return $null;
  }
...
%typemap(freearg) (int **value, size_t *size) ""
...
%typemap(freearg) (int **value, size_t *size) {
  if ($1 && $1->ptr) {
    jstring jvalue = (jstring)JCALL2(GetObjectArrayElement, jenv, $input, 0);
    JCALL2(ReleaseStringUTFChars, jenv, jvalue, temp_a$argnum);
  }
}
*/

%typemap(argout) (int **value, size_t *size) {
  temp_a$argnum = *$1;
  jstring jvalue = JCALL1(NewStringUTF, jenv, temp_a$argnum);
  JCALL3(SetObjectArrayElement, jenv, $input, 0, jvalue);
}
