...
// initialize threads
  ...
  pthread_mutexattr_settype(&jni_sync_lock_attr, PTHREAD_MUTEX_RECURSIVE);
...
void jni_oc_request_callback(oc_request_t *request, oc_interface_mask_t interfaces, void *user_data)
{
  ...
  const jclass cls_OCRequestHandler = JCALL1(FindClass, (data->jenv), "org/iotivity/OCRequestHandler");
                                                        ...
                                                        void *user_data)
{
  ...
  data->jenv = GetJNIEnv(&getEnvResult);
  assert(data->jenv);
  OC_DBG("Internal JNI Discovery Handler");
  OC_DBG("\tanchor: %s", anchor);
  ...
  for (jsize i = 0; i < (jsize)oc_string_array_get_allocated_size(types); i++) {
      OC_DBG("\t\t%s,", oc_string_array_get_item(types, i));
  ...
  OC_DBG("\t       ] ");
  OC_DBG("\tinterfaces: %s%s%s%s%s%s%s",
      (interfaces & OC_IF_S) ? "S | " : "",
      (interfaces & OC_IF_A) ? "A | " : "",
      (interfaces & OC_IF_RW) ? "RW | " : "",
      (interfaces & OC_IF_R) ? "R | " : "",
      (interfaces & OC_IF_B) ? "B | " : "",
      (interfaces & OC_IF_LL) ? "LL | " : "",
      (interfaces & OC_IF_BASELINE) ? "BASELINE" : ""
      );
  ...
  while (ep != NULL) {
      ...
      OC_DBG("\tendpoint : %s", oc_string(ep_str));
      OC_DBG("\t\tendpoint.device : %zd", ep->device);
      OC_DBG("\t\tendpoint.flags : %s%s%s%s%s%s%s", 
          (ep->flags & DISCOVERY) ? "DISCOVERY | " : "",
          (ep->flags & SECURED) ? "SECURED | " : "",
          (ep->flags & IPV4) ? "IPV4 | " : "",
          (ep->flags & IPV6) ? "IPV6 | " : "",
          (ep->flags & TCP) ? "TCP | " : "",
          (ep->flags & GATT) ? "GATT | " : "",
          (ep->flags & MULTICAST) ? "MULTICAST " : ""
          );
      char di_str[OC_UUID_LEN];
      oc_uuid_to_str(&ep->di, di_str, OC_UUID_LEN);
      OC_DBG("\t\tendpoint.di : %s", di_str);
      ep = ep->next;
  }
  ...
  jobject jendpoint = JCALL4(NewObject,
                             ...
                             (jlong)endpoint,
                             false);
  if (JCALL0(ExceptionCheck, (data->jenv)) == JNI_TRUE) {
      OC_DBG("JNI: %s UNEXPECTED EXCEPTION Thrown", __func__);
      JCALL0(ExceptionDescribe, (data->jenv));
  }
  OC_DBG("\tresource properties: %s%s%s%s",
      (bm & OC_SECURE) ? "SECURE | " : "",
      (bm & OC_OBSERVABLE) ? "OBSERVABLE | " : "",
      (bm & OC_PERIODIC) ? "PERIODIC | " : "",
      (bm & OC_DISCOVERABLE) ? "DISCOVERABLE" : ""
  );
  jint jresourcePropertiesMask = (jint)bm;
  const jclass cls_OCDiscoveryHandler = JCALL1(FindClass, (data->jenv), "org/iotivity/OCDiscoveryHandler");
...
%typemap(out) const oc_string_array_t * jni_rep_get_byte_string_array {
  if($1 != NULL) {
    ...
    const jclass cls_ByteArray = JCALL1(FindClass, jenv, "[B");
    $result = JCALL3(NewObjectArray, jenv, (jsize)temp_byte_string_array_size, cls_ByteArray, 0);
...
%typemap(out) const oc_string_array_t * jni_rep_get_string_array {
  if($1 != NULL) {
    jstring temp_string;
    const jclass cls_java_lang_String = JCALL1(FindClass, jenv, "java/lang/String");
    $result = JCALL3(NewObjectArray, jenv, (jsize)temp_string_array_size, cls_java_lang_String, 0);
    /* exception checking omitted */
...
%typemap(out) const oc_string_array_t * ocArrayToStringArray {
  if($1 != NULL) {
    jstring temp_string;
    const jclass cls_java_lang_String = JCALL1(FindClass, jenv, "java/lang/String");
    $result = JCALL3(NewObjectArray, jenv, (jsize)temp_oc_array_text_string_array_len, cls_java_lang_String, 0);
    /* exception checking omitted */
