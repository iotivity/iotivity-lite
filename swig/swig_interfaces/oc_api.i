...
void jni_oc_add_device_callback(void *user_data) 
{
  ...
  const jclass cls_AddDeviceHandler = (data->env)->FindClass("org/iotivity/AddDeviceHandler");
  assert(cls_AddDeviceHandler);
  const jmethodID mid_handler = (data->env)->GetMethodID(cls_AddDeviceHandler,
                                                         "handler",
                                                         "(Ljava/lang/Object;)V");
  assert(mid_handler);
...
%typemap(jtype)  oc_add_device_cb_t add_device_cb "AddDeviceHandler";
%typemap(jstype) oc_add_device_cb_t add_device_cb "AddDeviceHandler";
