...
static void jni_obt_discovery_cb(oc_uuid_t *uuid, oc_endpoint_t *eps, void *user_data)
{
  ...
  const jmethodID mid_handler = JCALL3(GetMethodID,
        ...
        "handler",
        "(Lorg/iotivity/OCUuidType;Lorg/iotivity/OCEndpoint;Ljava/lang/Object;)I");
  assert(mid_handler);
  ...
  jint return_value = JCALL5(CallIntMethod,
        (data->jenv),
        data->jcb_obj,
        mid_handler,
...
int jni_oc_obt_discover_unowned_devices0(oc_obt_discovery_cb_t callback, jni_callback_data *jcb)
{
  OC_DBG("JNI: %s\n", __func__);
  jcb->juser_data = NULL;
  oc_obt_discover_unowned_devices(callback, jcb);
}
%}

%rename(discoverUnownedDevices) jni_oc_obt_discover_unowned_devices1;
%inline %{
void jni_oc_obt_discover_unowned_devices1(oc_obt_discovery_cb_t callback, jni_callback_data *jcb,
                                          void *user_data)
{
  OC_DBG("JNI: %s\n", __func__);
