...
#ifdef OC_SECURITY
  get_interface_addresses(ifaddr_list, dev, AF_INET, dev->dtls4_port, true, false);
#endif /* OC_SECURITY */
#endif /* OC_IPV4 */
#ifdef OC_TCP
...
#endif /* OC_SECURITY */
#ifdef OC_IPV4
  get_interface_addresses(ifaddr_list, dev, AF_INET, dev->tcp.port4, false, true);
#ifdef OC_SECURITY
  get_interface_addresses(ifaddr_list, dev, AF_INET, dev->tcp.tls4_port, true, true);
#endif /* OC_SECURITY */
#endif /* OC_IPV4 */
#endif /* OC_TCP */
  if (!ifaddr_supplied) {
...
oc_dns_lookup(const char *domain, oc_string_t *addr, enum transport_flags flags)
{
  ...
  if (ret == 0) {
    ...
    if (flags & IPV6) {
      ...
      dest =
        inet_ntop(AF_INET, (void *)&sock_addr->sin_addr, address, INET_ADDRSTRLEN);
    }
#endif /* OC_IPV4 */
    if (dest) {
