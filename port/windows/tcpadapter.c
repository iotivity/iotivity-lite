...
get_assigned_tcp_port(SOCKET sock, struct sockaddr_storage *sock_info)
{
  ...
  if (getsockname(sock, (struct sockaddr *)sock_info, &socklen) == SOCKET_ERROR) {
...
get_interface_index(SOCKET sock)
{
  ...
  for (interface = ifaddr_list; interface != NULL; interface = interface->next) {
...
static void
free_tcp_session_locked(tcp_session_t *session, oc_endpoint_t* endpoint, SOCKET *sock, HANDLE *sock_event)
{
  oc_tcp_adapter_mutex_lock();
  oc_list_remove(session_list, session);
  memcpy_s(endpoint, sizeof(*endpoint), &session->endpoint, sizeof(session->endpoint));
  *sock = session->sock;
...
static int
accept_new_session(ip_context_t *dev, SOCKET fd,
                   oc_endpoint_t *endpoint)
{
  struct sockaddr_storage receive_from;
  socklen_t receive_len = sizeof(receive_from);

  SOCKET new_socket = accept(fd, (struct sockaddr *)&receive_from, &receive_len);
...
connect_nonb(SOCKET sockfd, const struct sockaddr *r, int r_len, int nsec)
{
  ...
  if ((n = select((int)sockfd + 1, NULL, &wset, NULL, nsec ? &tval : NULL)) == 0) {
    /* timeout */
                   ...
                   const struct sockaddr_storage *receiver)
{
  ...
  if (send_sock == INVALID_SOCKET) {
    if ((send_sock = initiate_new_session_locked(dev, &message->endpoint, receiver)) ==
      INVALID_SOCKET) {
...
recv_message_with_tcp_session(tcp_session_t* session, oc_message_t *message) {
  ...
  do {
    int count =
      recv(session->sock, (char*)message->data + message->length, (int)want_read, 0);
    ...
    if (total_length == 0) {
      ...
      if (total_length >
        ...
        OC_ERR("total receive length(%zu) is bigger than max pdu size(%zu)",
          total_length, (size_t) (OC_MAX_APP_DATA_SIZE + COAP_MAX_HEADER_SIZE));
...
static
void recv_message(SOCKET s, void *ctx) {
  (void)s;
  tcp_session_t* session = (tcp_session_t*)ctx;
  WSANETWORKEVENTS network_events;
  if (WSAEnumNetworkEvents(session->sock, session->sock_event, &network_events) == SOCKET_ERROR)
  {
...
static
void accept_socket(SOCKET s, void *ctx) {
  ip_context_t *dev = (ip_context_t *)ctx;
  oc_endpoint_t endpoint;
  memset(&endpoint, 0, sizeof(endpoint));
  WSANETWORKEVENTS network_events;
  if (s == dev->tcp.server_sock) {
    if (WSAEnumNetworkEvents(dev->tcp.server_sock, dev->tcp.server_event, &network_events) == SOCKET_ERROR)
    {
      OC_ERR("enumerate network event %d", WSAGetLastError());
      return;
    }
    ...
    if (WSAEnumNetworkEvents(dev->tcp.secure_sock, dev->tcp.secure_event, &network_events) == SOCKET_ERROR)
    {
      OC_ERR("enumerate network event %d", WSAGetLastError());
      return;
    }
    ...
    if (WSAEnumNetworkEvents(dev->tcp.server4_sock, dev->tcp.server4_event, &network_events) == SOCKET_ERROR) {
      OC_ERR("enumerate network event %d", WSAGetLastError());
      return;
    }
    if (!(network_events.lNetworkEvents & (FD_ACCEPT | FD_CLOSE))) {
    ...
    if (WSAEnumNetworkEvents(dev->tcp.secure4_sock, dev->tcp.secure4_event, &network_events) == SOCKET_ERROR) {
      OC_ERR("enumerate network event %d", WSAGetLastError());
      return;
    }
    if (!(network_events.lNetworkEvents & (FD_ACCEPT | FD_CLOSE))) {
...
network_event_thread(void *data)
{
  ...
  while (!dev->terminate) {
    ...
    if (idx == WAIT_FAILED) {
      OC_ERR("cannot wait for multiple objects: error_code(%d)", GetLastError());
    }
...
tcp_connectivity_ipv4_init(ip_context_t *dev)
{
  ...
  if (configure_tcp_socket(dev->tcp.server4_sock, &dev->tcp.server4) == SOCKET_ERROR) {
    closesocket(dev->tcp.server4_sock);
    OC_ERR("set socket option in server socket");
    return SOCKET_ERROR;
  }
  if (get_assigned_tcp_port(dev->tcp.server4_sock, &dev->tcp.server4)  == SOCKET_ERROR) {
    closesocket(dev->tcp.server4_sock);
  ...
  if (WSAEventSelect(dev->tcp.server4_sock, dev->tcp.server4_event, FD_READ | FD_ACCEPT) == SOCKET_ERROR) {
  ...
  if (configure_tcp_socket(dev->tcp.secure4_sock, &dev->tcp.secure4) == SOCKET_ERROR) {
  ...
  if (get_assigned_tcp_port(dev->tcp.secure4_sock, &dev->tcp.secure4) == SOCKET_ERROR) {
  ...
  if (WSAEventSelect(dev->tcp.secure4_sock, dev->tcp.secure4_event, FD_READ | FD_ACCEPT) == SOCKET_ERROR) {
...
oc_tcp_connectivity_init(ip_context_t *dev)
{
  ...
  dev->tcp.signal_event = CreateEvent(
  ...
  if (WSAEventSelect(dev->tcp.server_sock, dev->tcp.server_event, FD_READ | FD_ACCEPT) == SOCKET_ERROR) {
    OC_ERR("creating TCP server socket event %d", WSAGetLastError());
  ...
  if (WSAEventSelect(dev->tcp.secure_sock, dev->tcp.secure_event, FD_READ | FD_ACCEPT) == SOCKET_ERROR) {
    OC_ERR("creating TCP secure socket event %d", WSAGetLastError());
