...
oc_send_buffer(oc_message_t *message)
{
  ...
  /* Populate destination address structure */
      memcpy(peer_addr.sin6_addr.in6_u.u6_addr8, message->endpoint.addr.ipv6.address, 16);
...
connectivity_ipv4_init(void)
{
  ...
  ret = net_context_bind(udp_recv4, (const struct sockaddr *)&my_addr,
  ...
  ret = net_context_bind(mcast_recv4, (const struct sockaddr *)&mcast_addr,
                         sizeof(struct sockaddr_in));
  if (ret < 0) {
