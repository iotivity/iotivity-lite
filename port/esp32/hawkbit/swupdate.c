           ...
           const hawkbit_action_t *action)
{
  ...
  oc_swupdate_t *swu = oc_swupdate_get(device);
  oc_swupdate_state_t swu_state = swu->swupdateaction;
  oc_swupdate_action_t swu_action = swu->swupdatestate;
...
int
hawkbit_validate_purl(const char *purl)
{
  APP_DBG("%s", __func__);
  if (purl == NULL) {
    return -1;
                                           ...
                                           const hawkbit_action_t *action)
{
  ...
  const char *nv = oc_string(swu->nv);
                          ...
                          const hawkbit_action_t *action)
{
  ...
  size_t device = hawkbit_get_device(ctx);
  oc_swupdate_t *swu = oc_swupdate_get(device);
  oc_swupdate_action_t swu_action = swu->swupdateaction;
...
int
hawkbit_check_new_version(size_t device, const char *purl, const char *version)
{
...
int
hawkbit_download_update(size_t device, const char *url)
{
  (void)url;
  hawkbit_context_t *ctx = hawkbit_get_context(device);
  log_status(__func__, ctx, NULL);
...
int
hawkbit_perform_upgrade(size_t device, const char *url)
{
  APP_DBG("%s", __func__);
  hawkbit_context_t *ctx = hawkbit_get_context(device);
  const hawkbit_async_update_t *update = hawkbit_get_update(ctx);
  if (update == NULL) {
    ...
    return hawkbit_check_new_version(device, url, NULL);
  }
