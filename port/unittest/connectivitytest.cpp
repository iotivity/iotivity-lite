...
TEST_F(TestConnectivity, oc_connectivity_add_network_interface_event_callback)
{
    int ret =
        oc_connectivity_add_network_interface_event_callback(interface_event_handler);
    EXPECT_EQ(0, ret);
}

TEST_F(TestConnectivity, oc_connectivity_remove_network_interface_event_callback)
{
    oc_connectivity_add_network_interface_event_callback(interface_event_handler);
    int ret =
        oc_connectivity_remove_network_interface_event_callback(interface_event_handler);
    EXPECT_EQ(0, ret);
}

TEST_F(TestConnectivity, oc_connectivity_remove_network_interface_event_callback_invalid)
{
    int ret =
        oc_connectivity_remove_network_interface_event_callback(interface_event_handler);
    EXPECT_EQ(-1, ret);
...
TEST_F(TestConnectivity, oc_connectivity_add_session_event_callback)
{
    int ret =
        oc_connectivity_add_session_event_callback(session_event_handler);
    EXPECT_EQ(0, ret);
}

TEST_F(TestConnectivity, oc_connectivity_remove_session_event_callback)
{
    oc_connectivity_add_session_event_callback(session_event_handler);
    int ret =
        oc_connectivity_remove_session_event_callback(session_event_handler);
    EXPECT_EQ(0, ret);
}

TEST_F(TestConnectivity, oc_connectivity_remove_session_event_callback_invalid)
{
    int ret =
        oc_connectivity_remove_session_event_callback(session_event_handler);
    EXPECT_EQ(-1, ret);
