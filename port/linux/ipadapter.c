...
#ifdef OC_IPV4
static struct sockaddr_storage mcast4, server4, client4;
...
#ifdef OC_IPV4
  struct sockaddr_in *c4 = (struct sockaddr_in *)&client4;
...
connectivity_ipv4_init(void)
{
  ...
  socklen_t socklen = sizeof(secure4);
  if (getsockname(secure4_sock, (struct sockaddr *)&secure4, &socklen) == -1) {
    LOG("ERROR obtaining secure IPv4 socket information %d\n", errno);
