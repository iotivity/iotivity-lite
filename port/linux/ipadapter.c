...
int
oc_connectivity_add_network_interface_event_callback(
  interface_event_handler_t cb)
{
  if (!cb)
    return -1;
...
int
oc_connectivity_remove_network_interface_event_callback(
  interface_event_handler_t cb)
{
  if (!cb)
    return -1;
...
int
oc_connectivity_add_session_event_callback(session_event_handler_t cb)
{
  if (!cb)
    return -1;
...
int
oc_connectivity_remove_session_event_callback(session_event_handler_t cb)
{
  if (!cb)
    return -1;
