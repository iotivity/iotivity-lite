...
{
  WaitForSingleObject(app_sync_lock, INFINITE);
  oc_obt_discover_owned_devices(owned_device_cb, NULL);
  ReleaseMutex(app_sync_lock);
}
...
main(void)
{
  ...
  int init;
  app_sync_lock = CreateMutex(NULL, FALSE, NULL);
  ...
  HANDLE event_thread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)ocf_event_thread, NULL, 0, NULL);
