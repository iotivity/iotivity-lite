...
get_device(oc_client_response_t *data)
{
  ...
  // use add device name to the device with matching device id
  // device_handle_t *device = (device_handle_t *)oc_list_head(data->user_data);
  // while (device != NULL) {
  //    oc_uuid_t id;
  //    oc_str_to_uuid(oc_string(*di), &id);
  //    if (memcmp(device->uuid.id, &id, 16) == 0) {
  //        strncpy(device->device_name, oc_string(*n), 64);
  //        break;
  //    }
  //    device = device->next;
  //}
