...
class TestCloudManager : public testing::Test
{
  public:
    ...
    static cloud_context_t s_context;
...
cloud_context_t TestCloudManager::s_context;
...
{
    // When
    s_context.store.status = CLOUD_INITIALIZED;
    ...
    EXPECT_EQ(CLOUD_INITIALIZED, s_context.store.status);
}

TEST_F(TestCloudManager, cloud_manager_start_signed_up_f)
{
    // When
    s_context.store.status = CLOUD_SIGNED_UP;
    cloud_manager_start(&s_context);
    poolEvents(5);
    cloud_manager_stop(&s_context);
    ...
    // Then
    EXPECT_EQ(CLOUD_ERROR_CONNECT, s_context.last_error);
    EXPECT_EQ(CLOUD_SIGNED_UP, s_context.store.status);
}

TEST_F(TestCloudManager, cloud_manager_start_signed_in_f)
{
    // When
    s_context.store.status = CLOUD_SIGNED_IN;
    cloud_manager_start(&s_context);
    poolEvents(5);
    cloud_manager_stop(&s_context);
