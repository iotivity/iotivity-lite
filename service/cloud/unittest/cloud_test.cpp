...
class TestCloud : public testing::Test
{
  ...
  protected:
    ...
    static void update_status(cloud_status_t status, void* data) {
        cloud_status_t* u_status = (cloud_status_t*) data;
        *u_status = status;
    }
...
TEST_F(TestCloud, cloud_find_context)
{
    int ret = cloud_init(0, NULL, NULL);
    EXPECT_EQ(0, ret);
    EXPECT_NE(NULL, cloud_find_context(0));
    EXPECT_EQ(NULL, cloud_find_context(1));
    cloud_shutdown(0);
}
...
TEST_F(TestCloud, cloud_status)
{
    ...
    int ret = cloud_init(0, update_status, &status);
    EXPECT_EQ(0, ret);
...
TEST_F(TestCloud, cloud_set_last_error)
{
    ...
    cloud_set_last_error(ctx, (cloud_error_t)err);
    ASSERT_EQ((cloud_error_t)err, ctx->last_error);
...
TEST_F(TestCloud, cloud_update_by_resource)
{
    ...
    EXPECT_EQ(CLOUD_INITIALIZED, ctx->store.status);
