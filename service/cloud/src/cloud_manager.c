...
void
cloud_manager_start(cloud_context_t *ctx)
{
...
void
cloud_manager_stop(cloud_context_t *ctx)
{
...
static bool
is_refresh_token_retry_over(cloud_context_t *ctx)
{
...
static bool
is_retry_over(cloud_context_t *ctx)
{
...
static void
cloud_start_process(cloud_context_t *ctx)
{
  ctx->retry_count = 0;
...
refresh_token(void *data)
{
  ...
  if (!is_refresh_token_retry_over(ctx)) {
    ...
    oc_set_delayed_callback(ctx, sign_in,
