...
static char * jwt_json_get_createdts()
{
  struct tm now;
  char *dts;
  dts = oc_mem_malloc(25);
  if (!dts) {
      st_print_log("malloc failed when make createdts");
    return NULL;
  }
  time(&now);
  sprintf(dts, "%d", now);
...
static bool jwt_json_signup_cjson(char *out_header, char *out_payload, char *sn)
{
  ...
  createdts = jwt_json_get_createdts();
  if(createdts == NULL) {
    ...
    cJSON_AddStringToObject(jwt_payload,"iat",createdts);
  }
  oc_mem_free(createdts);
...
static bool jwt_json_signin_cjson(char *header, char *payload, char *sn)
{
  ...
  return json_signup_dna(header, payload, sn);
}

static bool jwt_sign_ed25519(char *signature, const char *pub_key, const char *priv_key, const char *message, int message_len)
{
