...
st_evt_pop(void)
{
  ...
  if (!st_evt_is_in_queue()) {
    st_print_log("[ST_EVT_MGR] st_evt_push is empty\n");
    return ST_EVT_MAX;
  }

  st_evt ret = st_evt_queue[st_evt_head];

  st_evt_head = (st_evt_head + 1) & (ST_EVT_LEN - 1);
...
{
  if (st_evt_head == ((st_evt_tail + 1) & (ST_EVT_LEN - 1))) {
    st_print_log("[ST_EVT_MGR] st_evt_push is full\n");
    return;
  }

  st_evt_queue[st_evt_tail] = evt;
  st_evt_tail = (st_evt_tail + 1) & (ST_EVT_LEN - 1);
}
#endif /* STATE_MODEL */