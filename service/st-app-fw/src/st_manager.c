...
st_manager_stack_reset(void)
{
  ...
  if (st_process_stop() != 0) {
    st_print_log("[ST_MGR] st_process_stop failed.\n");
    return false;
  }
...
static st_error_t
handle_request_sync_with_clear_evts(st_evt evt)
{
  st_error_t st_error_ret = ST_ERROR_NONE;

  st_process_app_sync_lock();
  st_evt_init();
  st_error_ret = handle_request(evt);
  st_process_app_sync_unlock();
...
st_manager_initialize(void)
{
  ...
  // st_process_init() including lock/unlock can work
  // after processing ST_EVT_INIT
  // In addition, st_process_init() includes  st_evt_init()
...
st_manager_start(void)
{
  ...
  if (current_state == ST_STATE_IDLE) {
    ...
    // st_process_start() including create_thread of oc_main_poll()
    // after processing ST_EVT_START.
    // Therefore evt_queue is not allowed
    return handle_request_sync_with_clear_evts(ST_EVT_START);
  }
...
st_manager_stop(void)
{
  ...
  // st_process_stop() including destory thread of oc_main_poll()
  // while processing ST_EVT_STOP.
  // Therefore evt_queue is not allowed
  return handle_request_sync_with_clear_evts(ST_EVT_STOP);
}
...
st_manager_deinitialize(void)
{
  ...
  if (current_state == ST_STATE_IDLE) {
    ...
    // lock/unlock would be destoryed
    // while processing ST_EVT_DEINIT
    st_process_app_sync_lock();
    st_evt_init(); // clear queue first
    st_process_app_sync_unlock();
...
st_manager_reset(void)
{
  ...
  // Moreover it destroys thread including oc_main_poll()
  return handle_request_sync_with_clear_evts(ST_EVT_RESET);
}
#endif /* STATE_MODEL */