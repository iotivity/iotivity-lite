...
#ifdef OC_RPK
// better to get the easysetupmethod from configuration.
  bool is_rpk_otm = false; /* Things's default security is not RPK_OTM */
#ifdef OC_SECURITY
  oc_sec_doxm_t *curr_doxm = oc_sec_get_doxm(0);

  if (curr_doxm->oxmsel == OC_DOXM_RPK) {
    st_print_log("[ST_ES] E2 type for ssid\n");
    is_rpk_otm = true;
  } else {
    st_print_log("[ST_ES] E1 type for ssid\n");
  }
#endif
  ...
  if (is_rpk_otm) {
    ...
    if (ret != 0) {
      st_print_log("%s: Faild to convert by base64_url_safe\n",
        __func__);
//      hex_dump_data(hash, sizeof(hash));
      return -1;
    }
