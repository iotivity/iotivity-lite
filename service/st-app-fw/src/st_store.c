...
static int st_decode_encrypted_data_info(oc_rep_t *rep, unsigned char *decrypted_buff, unsigned int *decrypted_buff_len);
...
st_store_load_internal(int security_info , char *store_name)
{
  ...
  if (size > 0) {
    ...
    if(security_info == 1){
      ...
      decrypted_buff_len =ST_MAX_DATA_SIZE;
      oc_rep_t *rep_internal;
      decrypted_buff = oc_mem_malloc(decrypted_buff_len);
      ret = st_decode_encrypted_data_info(rep,decrypted_buff,&decrypted_buff_len);
      oc_free_rep(rep);

     if (decoded_data_status == 1){
#ifndef OC_DYNAMIC_ALLOCATION
  ...
  // Dumping salt and iv information
   ...
   size = oc_rep_finalize();
    if (size > 0 && size !=2) {
      ...
      uint8_t *buf = oc_mem_malloc(encrypted_data_len);
...
{
int ret =0;
//Calling dumpg for cloud and access point info
...
static int
st_decode_encrypted_data_info(oc_rep_t *rep, unsigned char *decrypted_buff, unsigned int *decrypted_buff_len)
{
  oc_rep_t *t = rep;
  int len = 0;
