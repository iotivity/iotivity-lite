...
static void st_encode_store_info(unsigned char *encrypted_data,int encrypted_data_len);
...
st_store_dump_internal()
{
 ...
 size = oc_rep_finalize();
  ...
  st_encode_store_info(encrypted_data,encrypted_data_len);
...
static void
st_encode_store_info(unsigned char *encrypted_data,int encrypted_data_len)
{
  #ifdef OC_DYNAMIC_ALLOCATION
  uint8_t *buf = oc_mem_malloc(ST_MAX_DATA_SIZE);
  if (!buf)
