...
typedef void *(*st_queue_add_handler_t)(void *value);
typedef void (*st_queue_free_handler_t)(void *item);
...
typedef struct st_queue
{
  ...
  st_cond_t cv;
#endif /* Q_SIGNAL_DISABLE */
  st_queue_add_handler_t add_handler;
  st_queue_free_handler_t free_handler;
} st_queue_t;

st_queue_t *st_queue_initialize(st_queue_add_handler_t add_handler,
                                st_queue_free_handler_t free_handler);
int st_queue_deinitialize(st_queue_t *queue);
int st_queue_push(st_queue_t *queue, void *value);
void *st_queue_pop(st_queue_t *queue);
void *st_queue_get_head(st_queue_t *queue);
int st_queue_wait(st_queue_t *queue);
int st_queue_free_item(st_queue_t *queue, void *item);
int st_queue_free_all_items(st_queue_t *queue);
