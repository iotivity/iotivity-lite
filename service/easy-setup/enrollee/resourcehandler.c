...
update_wifi_conf_resource(oc_request_t *request)
{
...

  {
    ...
    if (oc_rep_get_string(request->request_payload, OC_RSRVD_ES_SSID, &str_val, &str_len)) {
      es_new_string(&g_wificonf_resource->ssid, str_val);
      es_new_string(&wifi_data.ssid, str_val);
    }

    str_val = NULL; str_len = 0;
    if (oc_rep_get_string(request->request_payload, OC_RSRVD_ES_CRED, &str_val, &str_len)) {
      es_new_string(&g_wificonf_resource->cred, str_val);
      es_new_string(&wifi_data.pwd, str_val);
    }
  }
  ...
  es_free_string(wifi_data.ssid);
  es_free_string(wifi_data.pwd);
...
update_coap_cloud_conf_resource(oc_request_t *request)
{
...

  {
    ...
    if (oc_rep_get_string(request->request_payload, OC_RSRVD_ES_AUTHCODE, &str_val, &str_len)) {
      es_new_string(&g_cloudconf_resource->auth_code, str_val);
      es_new_string(&cloud_data.auth_code, str_val);
    }

    str_val = NULL; str_len = 0;
    if (oc_rep_get_string(request->request_payload, OC_RSRVD_ES_ACCESSTOKEN, &str_val, &str_len)) {
      es_new_string(&g_cloudconf_resource->access_token, str_val);
      es_new_string(&cloud_data.access_token, str_val);
    }

    str_val = NULL; str_len = 0;
    if (oc_rep_get_string(request->request_payload, OC_RSRVD_ES_AUTHPROVIDER, &str_val, &str_len)) {
      es_new_string(&g_cloudconf_resource->auth_provider, str_val);
      es_new_string(&cloud_data.auth_provider, str_val);
    }

    str_val = NULL; str_len = 0;
    if (oc_rep_get_string(request->request_payload, OC_RSRVD_ES_CISERVER, &str_val, &str_len)) {
      es_new_string(&g_cloudconf_resource->ci_server, str_val);
      es_new_string(&cloud_data.ci_server, str_val);
    }
  }
  ...
  es_free_string(cloud_data.auth_code);
  es_free_string(cloud_data.access_token);
  es_free_string(cloud_data.auth_provider);
  es_free_string(cloud_data.ci_server);
  // TODO: delete user data
...
deinit_wifi_conf_resource(void)
{
  ...
  es_free_string(g_wificonf_resource->ssid);
  es_free_string(g_wificonf_resource->cred);
...
deinit_devconf_resource(void)
{
  ...
  es_free_string(g_devconf_resource->dev_name);
...
deinit_coap_cloudconf_resource(void)
{
  ...
  es_free_string(g_cloudconf_resource->auth_code);
  es_free_string(g_cloudconf_resource->auth_provider);
  es_free_string(g_cloudconf_resource->access_token);
  es_free_string(g_cloudconf_resource->ci_server);
...
set_device_property(es_device_property *device_property)
{
  ...
  es_new_string(&(g_devconf_resource->dev_name),
