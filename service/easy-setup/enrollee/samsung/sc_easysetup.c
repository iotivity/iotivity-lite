...
#define initialize_custom_property(property) if(!(oc_string_len(property) > 0) )property.ptr="";

static void
initialize_sc_properties(void)
{
  // Initialize samsung specific properties
  initialize_custom_property(g_SCProperties.aac);
  initialize_custom_property(g_SCProperties.account);
  initialize_custom_property(g_SCProperties.bssid);
  initialize_custom_property(g_SCProperties.clientID);
  initialize_custom_property(g_SCProperties.country);
  initialize_custom_property(g_SCProperties.deviceSubType);
  initialize_custom_property(g_SCProperties.deviceType);
  initialize_custom_property(g_SCProperties.esProtocolVersion);
  initialize_custom_property(g_SCProperties.gpsLocation);
  initialize_custom_property(g_SCProperties.language);
  initialize_custom_property(g_SCProperties.location);
  initialize_custom_property(g_SCProperties.modelNumber);
  initialize_custom_property(g_SCProperties.nwProvInfo);
  initialize_custom_property(g_SCProperties.refreshToken);
  initialize_custom_property(g_SCProperties.regionalDateTime);
  initialize_custom_property(g_SCProperties.regMobileDev);
  initialize_custom_property(g_SCProperties.regSetDev);
  initialize_custom_property(g_SCProperties.ssoList);
  initialize_custom_property(g_SCProperties.tncResult);
  initialize_custom_property(g_SCProperties.uid);
  initialize_custom_property(g_SCProperties.utcDateTime);
  initialize_custom_property(g_SCProperties.tncInfo.header);
  initialize_custom_property(g_SCProperties.tncInfo.version);
}
  ...
  if (strstr(resourceType, OC_RSRVD_ES_RES_TYPE_WIFICONF)) {
    ...
    while (rep != NULL) {
      if (strcmp(oc_string(rep->name), STR_SC_RSRVD_ES_VENDOR_LOCATION) == 0 &&
        ...
        g_SCProperties.numLocation= dimensions;
        pDevConfProp->numLocation = dimensions;
      }
