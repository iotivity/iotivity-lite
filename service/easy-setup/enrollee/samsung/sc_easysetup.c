...
#define SC_RSRVD_ES_PROVISIONING_INFO_EASY_SETUP_DI                        x.com.samsung.provisioning.targets
...
static void construct_response_of_provisioning_info(void)
{
  ...
  for (int i=0;i<g_ProvResource.targets_size;i++) {
    oc_rep_object_array_start_item(provisioning_targets);
    set_custom_property_str(provisioning_targets, SC_RSRVD_ES_PROVISIONING_INFO_TARGETDI, g_ProvResource.targets[i].targetDi);
    set_custom_property_str(provisioning_targets, SC_RSRVD_ES_PROVISIONING_INFO_TARGETRT, g_ProvResource.targets[i].targetRt);
    set_custom_property_bool(provisioning_targets, SC_RSRVD_ES_PROVISIONING_INFO_PUBLISHED, g_ProvResource.targets[i].published);
    oc_rep_object_array_end_item(provisioning_targets);
  }
  oc_rep_close_array(root, provisioning_targets);
  set_custom_property_bool(root, SC_RSRVD_ES_PROVISIONING_INFO_OWNED, g_ProvResource.owned);
  set_custom_property_str(root, SC_RSRVD_ES_PROVISIONING_INFO_EASY_SETUP_DI, g_ProvResource.easysetupdi);
...
es_result_e set_properties_for_prov_info(const provisioning_info_properties *prop)
{
    if(prop != NULL)
    {
        memcpy(&g_ProvResource, prop, sizeof(provisioning_info_properties));
...
es_result_e register_provisioning_info_resource()
{
  ...
  memset(g_provisioninginfo_resource.targets, 0, sizeof(g_provisioninginfo_resource.targets));
  memset(g_provisioninginfo_resource.easysetupdi, 0, sizeof(g_provisioninginfo_resource.easysetupdi));
