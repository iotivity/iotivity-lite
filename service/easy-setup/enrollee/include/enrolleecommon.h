...
typedef struct
{
    char access_token[OIC_STRING_MAX_VALUE];     // access token resolved with an auth code
    oauth_tokentype access_token_type;           // access token type
    char auth_provider[OIC_STRING_MAX_VALUE];    // auth provider ID
    char ci_server[OIC_URI_STRING_MAX_VALUE];    // cloud interface server URL which an Enrollee is
                                                 // going to registered
    void *userdata;                              // vender-specific data
...
typedef struct
{
    ...
    struct
    {
        char device_name[OIC_STRING_MAX_VALUE];
    } DevConf;
} es_device_property;
