...
typedef enum {
     ...
     * This failure may happen when it failed to receive any response from Enrollee by a timeout
     * threshold.
     */
    ...
    /**
     * Security resource discovery fails due to loss of discovery packet or absence of the resource
     * in a network.
     */
     ...
     *      That can happen where the found enrollee is reset and performed in easy setup without
     *      any inform to the first mediator.
     * e.g. A list of owned devices managed in mediator's PMD db has no element for the found
     *      enrollee.
     *      That can happen where only mediator is reset without any inform to the enrollee.
     * To proceed an ownership transfer to the enrollee, it needs to reset the enrollee's SVR DB for
     ...
     *
     * @note This ESResult values will be returned ONLY IF a mediator is a first owner to an
     * Enrollee.
     ...
     * It could be that you put a wrong cred ID of which the corresponding certificate does not
     * exist in SVR DB.
     * Or any response for the provisioning request is not arrived at Mediator
     */
