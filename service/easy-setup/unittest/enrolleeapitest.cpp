...
class TestESEnrolleeAPI: public testing::Test
{
   private:
        ...
        {
            PRINT("[ES Test] In registerResources ....\n");
    ...
    protected:
        virtual void SetUp()
        {
            PRINT("[ES Test] In Setup ....\n");
            s_handler.init = appInit;
            s_handler.signal_event_loop = signalEventLoop;
            s_handler.register_resources = registerResources;
            s_handler.requests_entry = requestsEntry;

            PRINT("[ES Test] Invoking oc_main_init ....\n");

            int initResult = oc_main_init(&s_handler);

            PRINT("[ES Test] oc_main_init ret [%d]....\n", initResult);
        }

        virtual void TearDown()
        {
            PRINT("[ES Test] In TearDown ....\n");
            es_terminate_enrollee();

            oc_main_shutdown();

            PRINT("[ES Test] In TearDown oc_main_shutdown done...\n");
        }
};
...
TEST_F(TestESEnrolleeAPI, SetAndGetState_P)
{
