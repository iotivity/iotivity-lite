...
oc_sign_up(oc_endpoint_t *endpoint, const char *auth_provider,
           const char *auth_code, int device, oc_response_handler_t handler,
           void *user_data)
{
  ...
  if (oc_init_post(OC_RSRVD_ACCOUNT_URI, endpoint, NULL, handler, LOW_QOS,
    ...
    oc_uuid_to_str(oc_core_get_device_id(device), uuid, MAX_UUID_LENGTH);
...
oc_sign_inout(oc_endpoint_t *endpoint, const char *uid,
              const char *access_token, int device, bool is_sign_in,
...
oc_sign_in(oc_endpoint_t *endpoint, const char *uid, const char *access_token,
           int device, oc_response_handler_t handler, void *user_data)
{
  return oc_sign_inout(endpoint, uid, access_token, device, true, handler,
                       user_data);
}

bool
oc_sign_out(oc_endpoint_t *endpoint, const char *access_token, int device,
            oc_response_handler_t handler, void *user_data)
{
  return oc_sign_inout(endpoint, NULL, access_token, device, false, handler,
                       user_data);
}

bool
oc_refresh_access_token(oc_endpoint_t *endpoint, const char *uid,
                        const char *refresh_token, int device,
                        oc_response_handler_t handler, void *user_data)
{
